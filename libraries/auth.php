...
class auth { 	
	...
	function is_manager1() {
		$sqlmgr = "SELECT d.`vAbbreviation` FROM hrd.`employee` a INNER JOIN hrd.`departement`
						d ON a.`iDivisionID` = d.`iDeptID`  WHERE a.cNip = '".$this->sess_auth->gNIP."' AND a.`lDeleted`='0'";
		$ms = $this->_ci->db->query($sqlmgr)->row_array();
		return $ms['c'] > 0;
	}
	function is_dir() {
