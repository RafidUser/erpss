...
class validasi_sop extends MX_Controller { 
    function __construct() {
        ...
        $this->title = 'SOP Signed';
        $this->url = 'validasi_sop';
        $this->urlH = 'bi_flow';        
    ...
    function index($action = '') {
        ...
        $grid->addFields('upload_dok','mKeterangan','iValidate','mValidate','dSubmit_validasi', 'mFeedback','form_history'); 
        ...
        // join tabel
        // $grid->setJoinTable('purchasing.poir_header', 'poir_header.cPONumber = biflow_sop.vKodeRef', 'inner');
        ...
        $grid->setQuery('biflow_sop.lDeleted = 0 ', null);

        $this->cKode = $this->input->get('cKode');            
        $grid->setInputGet('cKode', $this->cKode);            
        $grid->setQuery('biflow_sop.cKode', $this->input->get('cKode'));
        ...
        $grid->setRequired('mKeterangan', 'iValidate'); 
        ...
        switch ($action) {
            ...
            case 'createproses':
                ...
                // get cKode_sop
                $sql = "SELECT a.cKode_sop
                        FROM hrd.biflow_sop a
                        WHERE a.id = '".$lastId."' ";
                $row = $this->db->query($sql)->row_array();
                $cKode_sop = $row['cKode_sop'];
                        ...
                        foreach ($_FILES['upload_dok_upload_file']["error"] as $key => $error) {
                            if ($error == UPLOAD_ERR_OK) {
                                ...
                                if (move_uploaded_file($tmp_name, $file_path)) {
                                    $datainsert                         = array();
                                    $datainsert['cKode_sop']            = $cKode_sop;
                                    ...
                                    if ($this->db->insert('hrd.biflow_sop_file', $datainsert)){
                                    } else {
            ...
            case 'updateproses':
                ...
                if($isUpload){
                    ...
                    if (isset($_FILES['upload_dok_upload_file'])) {
                        ...
                        $this->db->where(array('cKode_sop' => $cKode_sop, 'iDeleted' => 0));
                        $this->db->update('hrd.biflow_sop_file', $update);
    ...
    function searchBox_validasi_sop_vKodeRef($rowData, $id) {
    ...
    function listBox_validasi_sop_upload_dok($value, $pk, $name, $rowData) {
        $rowData = get_object_vars($rowData);

        $sql = "SELECT *
                FROM hrd.biflow_sop_file a
                WHERE a.cKode_sop = '".$rowData['cKode_sop']."'
                AND a.iDeleted = 0
                ORDER BY a.id DESC ";
        $row = $this->db->query($sql)->row_array();
    ...
    function listBox_validasi_sop_validasi($value, $pk, $name, $rowData){
        $sql = "SELECT COUNT(case when a.iStatus_bi = 1 then 1 else null END) AS jumlah_no
                ...
                FROM hrd.biflow_sop_param a
                WHERE a.lDeleted = 0
                AND a.cKode_sop = '".$rowData->cKode_sop."' ";
        $row = $this->db->query($sql)->row_array();
    ...
    function listBox_validasi_sop_iValidate($value, $pk, $name, $rowData){
    ...
    function listBox_validasi_sop_del($value, $pk, $name, $rowData){
        // print_r($this->input->get());exit;
    ...
    function insertBox_validasi_sop_mKeterangan($field, $id) {
    ...
    function updateBox_validasi_sop_mKeterangan($field, $id, $value, $rowData) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<input type="hidden" name="isdraft" id="isdraft">';
        $o .= '<div class="col-sm-6">';
    ...
    function insertBox_validasi_sop_mValidate($field, $id) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<div class="col-sm-6">';
    ...
    function updateBox_validasi_sop_mValidate($field, $id, $value, $rowData) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<div class="col-sm-6">';
    ...
    function updateBox_validasi_sop_iValidate($field, $id, $value, $rowData) {
    ...
    function insertBox_validasi_sop_upload_dok($field, $id)
    {   
    ...
    function updateBox_validasi_sop_vModul($field, $id, $value, $rowData)
    {   
    ...
    function updateBox_validasi_sop_upload_dok($field, $id, $value, $rowData)
    {   
        // get data file
        $sql = "SELECT *
                FROM hrd.biflow_sop_file a
        ...
        $o = $linknya;
        // $o .= '<input type="file" id="'.$id.'" name="'.$field.'_upload_file[]" class="input_rows1 '.$required.' fileupload multi multifile" size="15" >';
        $o .= '<input type="hidden" name="cKode_sop" value="'.$rowData['cKode_sop'].'">';

        return $o;
    }
    
    function updateBox_validasi_sop_parameter($field, $id, $value, $rowData) {
        $data['table']      = "hrd.biflow_sop_param";
        $data['table_key']  = "cKode_sop";
        $data['id']         = $id;
        $data['field']      = $field;
        $data['url']        = $this->url;
    ...
    function insertBox_validasi_sop_validasi($field, $id) {
        $data['table']      = "hrd.biflow_sop_param";
        $data['table_key']  = "cKode_sop";
        $data['id']         = $id;
        $data['field']      = $field;
    ...
    function updateBox_validasi_sop_validasi($field, $id, $value, $rowData) {
        $data['table']      = "hrd.biflow_sop_param";
        $data['table_key']  = "cKode_sop";
        $data['id']         = $id;
        $data['field']      = $field;
    ...
    function insertBox_validasi_sop_dSubmit_validasi($field, $id) {
        $o = '<b>Auto after submit</b>';

        return $o;
    }
    
    function updateBox_validasi_sop_dSubmit_validasi($field, $id, $value, $rowData) {
        $o = '<b>Auto after submit</b>';
    ...
    function updateBox_validasi_sop_mFeedback($field, $id, $value, $rowData){
    ...
    function updateBox_validasi_sop_form_history($field, $id, $value, $rowData)
    {
        $data['id'] = $id;
        $data['field'] = $field;
        ...
        $sql = 'SELECT sop.mKeterangan, sop.dSubmit, ff.vFilename, sop.mFeedback, sop.mValidate, ff.vFilename_generate,
                    if(sop.iValidate = 1, "Valid", if(sop.iValidate = 2, "Not Valid", "Need To Be Validate")) AS Validasi,
                    if(sop.iSubmit = 1 AND sop.lDeleted = 0, "Submitted", IF (sop.lDeleted = 1, "Deleted", "Need To Be Submit")) AS Status
                FROM hrd.biflow_sop_log sop
                LEFT JOIN hrd.biflow_sop_file_log ff ON ff.cKode_sop = sop.cKode_sop 
                WHERE sop.cKode_sop = "' . $rowData['cKode_sop'] . '"
                AND sop.iSubmit NOT IN (0)
                GROUP BY sop.id
                ORDER BY sop.dSubmit ASC
        ';

        $datasql = $this->db->query($sql)->result_array();
        $data['datasql'] = $datasql;

        $o = $this->load->view('partial/modul/bi_flow_sop_history', $data, true);
        return $o;
    }
    ...
    function before_insert_processor($row, $postData) {
        ...
        $postData['cKode_sop']  = $this->generateCodeDetail($postData['cKode']);
    ...
    function before_update_processor($row, $postData) {
        ...
        $kode_sop = $datanya['cKode_sop'];

        $sql = "SELECT * 
                FROM biflow_sop_log
                WHERE cKode_sop =  '".$kode_sop."'
                ORDER BY id DESC
                LIMIT 1
            ";
        ...
        if ($postData['isdraft'] == 'true') {
            ...
            foreach ($rows as $key => $row) {
                ...
                foreach ($row as $k => $r) {
                    ...
                    // $update['cKode_sop_log'] = $cKode_log;
                    $update[$k] = $r;
                ...
                $this->db->where(array('id' =>  $get_id['id'], 'cKode_sop' =>  $kode_sop, 'lDeleted' => 0/*, 'cKode_sop_log' => */));
                $this->db->update('hrd.biflow_sop_log', $update);
            }
    ...
    function after_insert_processor($fields, $id, $postData){
        ...
        foreach ($status_bi as $key => $bi) {
            ...
            $insBi['cKode_sop']         = $postData['cKode_sop'];
            ...
            if($this->db->insert('hrd.biflow_sop_param', $insBi)){
            }else{
                echo $this->db->last_query();
            }
        }*/
    ...
    function after_update_processor($fields, $id, $postData) {
        ...
        $this->db->where('cKode_sop', $postData['cKode_sop']);
        $this->db->update('hrd.biflow_sop_param', $upDel);
    ...
    function generateCodeDetail($kodeHeader){
        ...
        if(!empty($row)){
            $nilai = str_replace($kodeHeader.'-', "", $row['cKode_sop']);
            $int = intval($nilai);
            if ($int > 0) {
                $nilai = $int + 1;
            } else {
    ...
    function generateCodeDetail_2($kodeHeader){
        $sql = "SELECT a.cKode_sop_i
                FROM hrd.biflow_sop_item a
                WHERE a.lDeleted = 0
                AND a.cKode_sop = '".$kodeHeader."'
                ORDER BY a.id DESC
                LIMIT 1 ";
        $row = $this->db->query($sql)->row_array();

        if(!empty($row)){
            $nilai = str_replace($kodeHeader.'-', "", $row['cKode_sop_i']);
            $int = intval($nilai);
            if ($int > 0) {
                $nilai = $int + 1;
            } else {
