...
class uat_oat extends MX_Controller { 
    function __construct() {
        ...
        $this->title = 'Informasi Output Akhir Transaksi';
        $this->url = 'uat_oat';
        $this->urlH = 'bi_flow';        
    ...
    function index($action = '') {
        ...
        $grid->addFields('upload_dok','mKeterangan','iValidate_uat','mValidate_uat','dSubmit_uat'); 
        ...
        // join tabel
        // $grid->setJoinTable('purchasing.poir_header', 'poir_header.cPONumber = biflow_oat.vKodeRef', 'inner');
        ...
        $grid->setQuery('biflow_oat.lDeleted = 0 ', null);

        $this->cKode = $this->input->get('cKode');            
        $grid->setInputGet('cKode', $this->cKode);            
        $grid->setQuery('biflow_oat.cKode', $this->input->get('cKode'));
        ...
        switch ($action) {
            ...
            case 'createproses':
                ...
                // get cKode_oat
                $sql = "SELECT a.cKode_oat
                        FROM hrd.biflow_oat a
                        WHERE a.id = '".$lastId."' ";
                $row = $this->db->query($sql)->row_array();
                $cKode_oat = $row['cKode_oat'];
                        ...
                        foreach ($_FILES['upload_dok_upload_file']["error"] as $key => $error) {
                            if ($error == UPLOAD_ERR_OK) {
                                ...
                                if (move_uploaded_file($tmp_name, $file_path)) {
                                    $datainsert                         = array();
                                    $datainsert['cKode_oat']            = $cKode_oat;
                                    ...
                                    if ($this->db->insert('hrd.biflow_oat_file', $datainsert)){
                                    } else {
            ...
            case 'updateproses':
                ...
                if($isUpload){
                    ...
                    if (isset($_FILES['upload_dok_upload_file'])) {
                        ...
                        $this->db->where(array('cKode_oat' => $cKode_oat, 'iDeleted' => 0));
                        $this->db->update('hrd.biflow_oat_file', $update);
    ...
    function searchBox_uat_oat_vKodeRef($rowData, $id) {
    ...
    function listBox_uat_oat_upload_dok($value, $pk, $name, $rowData) {
        $rowData = get_object_vars($rowData);

        $sql = "SELECT *
                FROM hrd.biflow_oat_file a
                WHERE a.cKode_oat = '".$rowData['cKode_oat']."'
                AND a.iDeleted = 0
                ORDER BY a.id DESC ";
        $row = $this->db->query($sql)->row_array();
    ...
    function listBox_uat_oat_validasi($value, $pk, $name, $rowData){
        $sql = "SELECT COUNT(case when a.iStatus_bi = 1 then 1 else null END) AS jumlah_no
                ...
                FROM hrd.biflow_oat_param a
                WHERE a.lDeleted = 0
                AND a.cKode_oat = '".$rowData->cKode_oat."' ";
        $row = $this->db->query($sql)->row_array();
    ...
    function listBox_uat_oat_iValidate_uat($value, $pk, $name, $rowData){
    ...
    function listBox_uat_oat_del($value, $pk, $name, $rowData){
        // print_r($this->input->get());exit;
    ...
    function insertBox_uat_oat_mKeterangan($field, $id) {
    ...
    function updateBox_uat_oat_mKeterangan($field, $id, $value, $rowData) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<input type="hidden" name="isdraft" id="isdraft">';
        $o .= '<div class="col-sm-6">';
    ...
    function insertBox_uat_oat_mValidate_uat($field, $id) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<div class="col-sm-6">';
        $o .= '<textarea id="'.$id.'" name="'.$field.'" class="ckeditorField form-control" colspan="2"></textarea>';
        $o .= '<script>
    ...
    function updateBox_uat_oat_mValidate_uat($field, $id, $value, $rowData) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<div class="col-sm-6">';
    ...
    function updateBox_uat_oat_iValidate_uat($field, $id, $value, $rowData) {
    ...
    function insertBox_uat_oat_upload_dok($field, $id)
    {   
    ...
    function updateBox_uat_oat_vModul($field, $id, $value, $rowData)
    {   
    ...
    function updateBox_uat_oat_upload_dok($field, $id, $value, $rowData)
    {   
        // get data file
        $sql = "SELECT *
                FROM hrd.biflow_oat_file a
        ...
        $o = $linknya;
        // $o .= '<input type="file" id="'.$id.'" name="'.$field.'_upload_file[]" class="input_rows1 '.$required.' fileupload multi multifile" size="15" >';
        $o .= '<input type="hidden" name="cKode_oat" value="'.$rowData['cKode_oat'].'">';

        return $o;
    }
    
    function updateBox_uat_oat_parameter($field, $id, $value, $rowData) {
        $data['table']      = "hrd.biflow_oat_param";
        $data['table_key']  = "cKode_oat";
        $data['id']         = $id;
        $data['field']      = $field;
        $data['url']        = $this->url;
    ...
    function insertBox_uat_oat_validasi($field, $id) {
        $data['table']      = "hrd.biflow_oat_param";
        $data['table_key']  = "cKode_oat";
        $data['id']         = $id;
        $data['field']      = $field;
    ...
    function updateBox_uat_oat_validasi($field, $id, $value, $rowData) {
        $data['table']      = "hrd.biflow_oat_param";
        $data['table_key']  = "cKode_oat";
        $data['id']         = $id;
        $data['field']      = $field;
    ...
    function insertBox_uat_oat_dSubmit_uat($field, $id) {
        $o = '<b>Auto after submit</b>';

        return $o;
    }
    
    function updateBox_uat_oat_dSubmit_uat($field, $id, $value, $rowData) {
        $o = '<b>Auto after submit</b>';
    ...
    function before_insert_processor($row, $postData) {
        ...
        $postData['cKode_oat']  = $this->generateCodeDetail($postData['cKode']);
    ...
    function after_insert_processor($fields, $id, $postData){
        ...
        foreach ($status_bi as $key => $bi) {
            ...
            $insBi['cKode_oat']         = $postData['cKode_oat'];
            ...
            if($this->db->insert('hrd.biflow_oat_param', $insBi)){
            }else{
                echo $this->db->last_query();
            }
        }*/
    ...
    function after_update_processor($fields, $id, $postData) {
        ...
        $this->db->where('cKode_oat', $postData['cKode_oat']);
        $this->db->update('hrd.biflow_oat_param', $upDel);
    ...
    function generateCodeDetail($kodeHeader){
        ...
        if(!empty($row)){
            $nilai = str_replace($kodeHeader.'-', "", $row['cKode_oat']);
            $int = intval($nilai);
            if ($int > 0) {
                $nilai = $int + 1;
            } else {
    ...
    function generateCodeDetail_2($kodeHeader){
        $sql = "SELECT a.cKode_oat_i
                FROM hrd.biflow_oat_item a
                WHERE a.lDeleted = 0
                AND a.cKode_oat = '".$kodeHeader."'
                ORDER BY a.id DESC
                LIMIT 1 ";
        $row = $this->db->query($sql)->row_array();

        if(!empty($row)){
            $nilai = str_replace($kodeHeader.'-', "", $row['cKode_oat_i']);
            $int = intval($nilai);
            if ($int > 0) {
                $nilai = $int + 1;
            } else {
