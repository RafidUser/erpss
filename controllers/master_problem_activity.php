<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
    class master_problem_activity extends MX_Controller {
    private $sess_auth;
    private $dbset;
    function __construct() {
        ...
        $this->url = 'master_problem_activity'; 
    }
    ...
    function index($action = '') {
        ...
        $grid = new Grid;		
        $grid->setTitle('Problem Activity');		
        $grid->setTable('ss.problem_act');		
        $grid->setUrl('master_problem_activity');		
        $grid->addList('typeId','activity_id','cUpdatedBy','lDeleted' );//'lPersen', 'yPersen',
		...
		$grid->addFields('typeId', 'activity_id','tCreated','tUpdated','cUpdatedBy','lDeleted' );
		
		$grid->changeFieldType('lDeleted','combobox', '', array(''=>'-- All --', 0=>'Active', 1=>'Deleted'));
		
        $grid->setQuery('ss.problem_act.lDeleted', 0);
       
	//set search
        $grid->setSearch('typeId', 'activity_id');
		
        //set required
        $grid->setRequired('typeId','activity_id','tCreated','tUpdated','cUpdatedBy' );//Field yg mandatori
	...
	function listBox_master_problem_activity_typeId($value, $pk, $name, $rowData) {

		$sql = "SELECT a.typeName from hrd.ss_support_type a where a.typeId = '{$value}'";
	...
	function listBox_master_problem_activity_activity_id($value, $pk, $name, $rowData) {
	...
	function listBox_master_problem_activity_cUpdatedBy($value, $pk, $name, $rowData) {
	...
	public function insertbox_master_problem_activity_cUpdatedBy($field, $id) {
	...
	public function updatebox_master_problem_activity_cUpdatedBy($field, $id, $value, $data) {
	...
	public function insertbox_master_problem_activity_tCreated($field, $id) {
        $o  = "<input type='hidden' name='".$field."' id='".$id."' />";       
        
        return $o;
    }
	
	public function updatebox_master_problem_activity_tCreated($field, $id, $value, $data) {
	...
	public function insertbox_master_problem_activity_tUpdated($field, $id) {
        $o  = "<input type='hidden' name='".$field."' id='".$id."' />";       
        
        return $o;
    }
	
	public function updatebox_master_problem_activity_tUpdated($field, $id, $value, $data) {
	...
	public function insertbox_master_problem_activity_typeId($field, $id) {
      if ($this->input->get('action') == 'view') {
            $sql = "Select typeName as vDescription 
                    from hrd.ss_support_type where typeId = '{$id}'";
            $query = $this->dbset->query($sql);
            if ($query->num_rows() > 0) {
                $row = $query->row();
                $o = $row->vDescription;
            ...
            $sql = "Select typeId as typeId, typeName as vDescription 
                    from hrd.ss_support_type order by typeName";
            $query = $this->dbset->query($sql);
            if ($query->num_rows() > 0) {
	...
	function searchBox_master_problem_activity_typeId($field, $id) {
		$o  = "<select name='".$field."' id='".$id."'>";
	...
	public function updateBox_master_problem_activity_typeId($field, $id, $value) {
        
        if ($this->input->get('action') == 'view') {
            $sql = "Select typeName as vDescription 
                    from hrd.ss_support_type where typeId = '{$value}'";
            $query = $this->dbset->query($sql);
            if ($query->num_rows() > 0) {
                $row = $query->row();
                $o = $row->vDescription;
	...
	function searchBox_master_problem_activity_activity_id($field, $id) {
		$o  = "<select name='".$field."' id='".$id."'>";
            $o .= "<option value=''>Pilih</option>";
	...
	public function insertbox_master_problem_activity_activity_id($field, $id) {
        if ($this->input->get('action') == 'view') {
	...
	public function updateBox_master_problem_activity_activity_id($field, $id, $value) {
	...
	public function after_insert_processor($fields, $id, $post) {
        ...
        $sql = "Update _dbname_._tblname_ set tCreated = CURRENT_TIMESTAMP, cCreatedBy='".$cNip."', 
                dUpdated = CURRENT_TIMESTAMP, typeId='".$cNip."' where id='".$id."'";
    ...
    public function after_update_processor($fields, $id, $post) {
        ...
        $sql = "Update _dbname_._tblname_ set tUpdated = CURRENT_TIMESTAMP, typeId='".$cNip."' 
            where id='".$id."'";
        $this->dbset->query($sql);
    }
	*/
