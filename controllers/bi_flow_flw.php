...
class bi_flow_flw extends MX_Controller { 
    ...
    function index($action = '') {
        ...
        // join tabel
        // $grid->setJoinTable('purchasing.poir_header', 'poir_header.cPONumber = biflow_flw.vKodeRef', 'inner');
        ...
        $grid->setQuery('biflow_flw.lDeleted = 0 ', null);

        $this->cKode = $this->input->get('cKode');            
        $grid->setInputGet('cKode', $this->cKode);            
        $grid->setQuery('biflow_flw.cKode', $this->input->get('cKode'));
        ...
        $grid->setRequired('upload_dok','mKeterangan','feedback');
        ...
        switch ($action) {
            ...
            case 'createproses':
                ...
                // get cKode_flw
                $sql = "SELECT a.cKode_flw
                        FROM hrd.biflow_flw a
                        WHERE a.id = '".$lastId."' ";
                $row = $this->db->query($sql)->row_array();
                $cKode_flw = $row['cKode_flw'];

                $cKode_flw_log = $cKode . '-HIST-' . $nowDate;
                // echo print_r($get_id);exit;
                        ...
                        foreach ($_FILES['upload_dok_upload_file']["error"] as $key => $error) {
                            if ($error == UPLOAD_ERR_OK) {
                                ...
                                if (move_uploaded_file($tmp_name, $file_path)) {
                                    $datainsert                         = array();
                                    $datainsert['cKode_flw']            = $cKode_flw;
                                    ...
                                    $insertdataa                         = array();
                                    $insertdataa['cKode_flw']            = $cKode_flw;
                                    $insertdataa['cKode_flw_log']        = $cKode_flw_log;
                                    ...
                                    if (($this->db->insert('hrd.biflow_flw_file', $datainsert)) && ($this->db->insert('hrd.biflow_flw_file_log', $insertdataa))){
                                    } else {
            ...
            case 'updateproses':
                ...
                if($isUpload){
                    ...
                    if (isset($_FILES['upload_dok_upload_file'])) {
                        $sql = "SELECT * 
                                        FROM biflow_flw_file
                                        WHERE cKode_flw =  '".$cKode_flw."'
                                        ORDER BY id DESC
                                        LIMIT 1
                                    ";
                        ...
                        $this->db->where(array('cKode_flw' => $cKode_flw, 'iDeleted' => 0));
                        $this->db->update('hrd.biflow_flw_file', $update);

                        $up['iDeleted']             = 0;
                        $up['dUpdate']              = date('Y-m-d H:i:s');
                        $up['cUpdate']              = $this->user->gNIP;

                        $this->db->where(array('id' => $get_id['id'], 'cKode_flw' => $cKode_flw, 'iDeleted' => 0));
                        $this->db->update('hrd.biflow_flw_file_log', $up);
            ...
            case 'delete':
                $postData = $this->input->post();
                    ...
                    $kode_flw = $datanya['cKode_flw'];

                    $sql = "SELECT * 
                                    FROM biflow_flw_log
                                    WHERE cKode_flw =  '".$kode_flw."'
                                    ORDER BY id DESC
                                    LIMIT 1
                                ";
                    $get_id = $this->db->query($sql)->row_array();
                    ...
                    $this->db->where(array('id' => $get_id['id'], 'cKode_flw' => $kode_flw));
                    $this->db->update('hrd.biflow_flw_log', $up);
                    // echo print_r($kode_flw);exit;
    ...
    function searchBox_bi_flow_flw_vKodeRef($rowData, $id) {
    ...
    function listBox_bi_flow_flw_upload_dok($value, $pk, $name, $rowData) {
        $rowData = get_object_vars($rowData);

        $sql = "SELECT *
                FROM hrd.biflow_flw_file a
                WHERE a.cKode_flw = '".$rowData['cKode_flw']."'
                AND a.iDeleted = 0
                ORDER BY a.id DESC ";
        $row = $this->db->query($sql)->row_array();
    ...
    function listBox_bi_flow_flw_validasi($value, $pk, $name, $rowData){
        $sql = "SELECT COUNT(case when a.iStatus_bi = 1 then 1 else null END) AS jumlah_no
                ...
                FROM hrd.biflow_flw_param a
                WHERE a.lDeleted = 0
                AND a.cKode_flw = '".$rowData->cKode_flw."' ";
        $row = $this->db->query($sql)->row_array();
    ...
    function listBox_bi_flow_flw_del($value, $pk, $name, $rowData){
        // print_r($this->input->get());exit;
    ...
    function insertBox_bi_flow_flw_mKeterangan($field, $id) {
    ...
    function updateBox_bi_flow_flw_mKeterangan($field, $id, $value, $rowData) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<input type="hidden" name="isdraft" id="isdraft">';
        $o .= '<div class="col-sm-6">';
    ...
    function insertBox_bi_flow_flw_upload_dok($field, $id)
    {   
        $o = '<input type="file" id="'.$id.'" name="'.$field.'_upload_file[]" class="input_rows1 fileupload multi multifile" size="15 required" >';

        return $o;
    }

    function updateBox_bi_flow_flw_upload_dok($field, $id, $value, $rowData)
    {   
        // get data file
        $sql = "SELECT *
                FROM hrd.biflow_flw_file a
        ...
        $o = $linknya;
        $o .= '<input type="file" id="'.$id.'" name="'.$field.'_upload_file[]" class="input_rows1 fileupload multi multifile" size="15 required" >';
        $o .= '<input type="hidden" name="cKode_flw" value="'.$rowData['cKode_flw'].'">';

        return $o;
    }

    function insertBox_bi_flow_flw_parameter($field, $id) {
        $data['table']      = "hrd.biflow_flw_param";
        $data['table_key']  = "cKode_flw";
        $data['id']         = $id;
        $data['field']      = $field;
        $data['url']        = $this->url;
    ...
    function updateBox_bi_flow_flw_parameter($field, $id, $value, $rowData) {
        $data['table']      = "hrd.biflow_flw_param";
        $data['table_key']  = "cKode_flw";
        $data['id']         = $id;
        $data['field']      = $field;
    ...
    function insertBox_bi_flow_flw_dSubmit($field, $id) {
        $o = '<b>Auto after submit</b>';

        return $o;
    }
    
    function updateBox_bi_flow_flw_dSubmit($field, $id, $value, $rowData) {
        $o = '<b>Auto after submit</b>';
    ...
    function insertBox_bi_flow_flw_mFeedback($field, $id) {
    ...
    function updateBox_bi_flow_flw_mFeedback($field, $id, $value, $rowData) {
        // print_r($this->input->get());exit;
        $data = $this->input->get();
        $id = $data['id'];
    ...
    function insertBox_bi_flow_flw_form_history($field, $id)
    {
        $o = '<b> - </b>';
    ...
    function updateBox_bi_flow_flw_form_history($field, $id, $value, $rowData)
    {
        $data['id'] = $id;
        $data['field'] = $field;
        ...
        $sql = 'SELECT flw.mKeterangan/*COALESCE(ff.tKeterangan,flw.mKeterangan) AS ket*/, ff.vFilename, flw.dSubmit,
                    flw.mValidate, flw.mFeedback, ff.vFilename_generate,                    
                    if(flw.iValidate = 1, "Valid", if(flw.iValidate = 2, "Not Valid", "Need To Be Validate")) AS Validasi,
                    if(flw.iSubmit = 1 AND flw.lDeleted = 0, "Submitted", IF (flw.lDeleted = 1, "Deleted", "Need To Be Submit")) AS Status
                FROM hrd.biflow_flw_log flw
                LEFT JOIN hrd.biflow_flw_file_log ff ON ff.cKode_flw = flw.cKode_flw
                WHERE flw.cKode_flw = "' . $rowData['cKode_flw'] . '"
                AND flw.iSubmit NOT IN (0)
                GROUP BY flw.id
                ORDER BY flw.dSubmit ASC
        ';
    ...
    function before_insert_processor($row, $postData) {
        ...
        $postData['cKode_flw']  = $this->generateCodeDetail($postData['cKode']);
        ...
        if ($postData['isdraft'] == 'true') {
            ...
            foreach ($rows as $key => $row) {
                ...
                foreach ($row as $k => $r) {
                    ...
                    $insert['cKode_flw_log'] = $cKode_log;
                    $insert[$k] = $r;
                ...
                unset($insert['id']);
                // $this->db->insert('hrd.biflow_flw_log', $insert);
            ...
            foreach ($rows as $key => $row) {
                ...
                foreach ($row as $k => $r) {
                    // $insert[$keyHist] = $kodeHist;
                    $update['cKode_flw_log'] = $cKode_log;
                    $update[$k] = $r;
                ...
                $sql = "SELECT * 
                            ...
                            ";
                $get_id = $this->db->query($sql)->row_array();
                $this->db->insert('hrd.biflow_flw_log', $update);

                // echo print_r($get_id);
                // exit;

                // $this->db->where(array('id' =>  $get_id['id'], 'cKode_flw' =>  $kode_flw, 'lDeleted' => 0/*, 'cKode_br_log' => */));
                // $this->db->update('hrd.biflow_flw_log', $update);
                // echo print_r($update);
                // exit;
            }
        }
    ...
    function after_update_processor($fields, $id, $postData) {
        ...
        // $this->db->where('cKode_flw', $postData['cKode_flw']);
        // $this->db->update('hrd.biflow_flw_param', $upDel);
        ...
        //     $insBi['cKode_flw']         = $postData['cKode_flw'];
        ...
        //     if($this->db->insert('hrd.biflow_flw_param', $insBi)){
        //     }else{
        //         echo $this->db->last_query();
        //     }
        // }
    ...
    function generateCodeDetail($kodeHeader){
        ...
        if(!empty($row)){
            $nilai = str_replace($kodeHeader.'-', "", $row['cKode_flw']);
            $int = intval($nilai);
            if ($int > 0) {
                $nilai = $int + 1;
            } else {
    ...
    function generateCodeDetail_2($kodeHeader){
        $sql = "SELECT a.cKode_flw_i
                FROM hrd.biflow_flw_item a
                WHERE a.lDeleted = 0
                AND a.cKode_flw = '".$kodeHeader."'
                ORDER BY a.id DESC
                LIMIT 1 ";
        $row = $this->db->query($sql)->row_array();

        if(!empty($row)){
            $nilai = str_replace($kodeHeader.'-', "", $row['cKode_flw_i']);
            $int = intval($nilai);
            if ($int > 0) {
                $nilai = $int + 1;
            } else {
