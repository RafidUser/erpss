...
class validasi_for extends MX_Controller { 
                        ...
                        foreach ($_FILES['upload_dok_upload_file']["error"] as $key => $error) {
                            if ($error == UPLOAD_ERR_OK) {
                                ...
                                if (move_uploaded_file($tmp_name, $file_path)) {
                                    ...
                                    if ($this->db->insert('hrd.biflow_for_file', $datainsert)){
                                    } else {
    ...
    function searchBox_validasi_for_vKodeRef($rowData, $id) {
    ...
    function listBox_validasi_for_upload_dok($value, $pk, $name, $rowData) {
    ...
    function listBox_validasi_for_validasi($value, $pk, $name, $rowData){
    ...
    function listBox_validasi_for_iValidate($value, $pk, $name, $rowData){
    ...
    function listBox_validasi_for_del($value, $pk, $name, $rowData){
        // print_r($this->input->get());exit;
    ...
    function insertBox_validasi_for_mKeterangan($field, $id) {
    ...
    function updateBox_validasi_for_mKeterangan($field, $id, $value, $rowData) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<input type="hidden" name="isdraft" id="isdraft">';
        $o .= '<div class="col-sm-6">';
    ...
    function insertBox_validasi_for_mValidate($field, $id) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<div class="col-sm-6">';
        $o .= '<textarea id="'.$id.'" name="'.$field.'" class="ckeditorField form-control" colspan="2"></textarea>';
        $o .= '<script>
    ...
    function updateBox_validasi_for_mValidate($field, $id, $value, $rowData) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<div class="col-sm-6">';
    ...
    function updateBox_validasi_for_iValidate($field, $id, $value, $rowData) {
    ...
    function insertBox_validasi_for_upload_dok($field, $id)
    {   
    ...
    function updateBox_validasi_for_vModul($field, $id, $value, $rowData)
    {   
    ...
    function updateBox_validasi_for_upload_dok($field, $id, $value, $rowData)
    {   
        // get data file
        $sql = "SELECT *
                FROM hrd.biflow_for_file a
    ...
    function updateBox_validasi_for_parameter($field, $id, $value, $rowData) {
    ...
    function insertBox_validasi_for_validasi($field, $id) {
        $data['table']      = "hrd.biflow_for_param";
        $data['table_key']  = "cKode_for";
        $data['id']         = $id;
        $data['field']      = $field;
    ...
    function updateBox_validasi_for_validasi($field, $id, $value, $rowData) {
        $data['table']      = "hrd.biflow_for_param";
        $data['table_key']  = "cKode_for";
        $data['id']         = $id;
        $data['field']      = $field;
    ...
    function insertBox_validasi_for_dSubmit_validasi($field, $id) {
        $o = '<b>Auto after submit</b>';

        return $o;
    }
    
    function updateBox_validasi_for_dSubmit_validasi($field, $id, $value, $rowData) {
        $o = '<b>Auto after submit</b>';
    ...
    function updateBox_validasi_for_mFeedback($field, $id, $value, $rowData){
    ...
    function updateBox_validasi_for_form_history($field, $id, $value, $rowData)
    {
        $data['id'] = $id;
        $data['field'] = $field;
        ...
        $o = $this->load->view('partial/modul/bi_flow_for_history', $data, true);
        return $o;
    }
    ...
    function before_update_processor($row, $postData) {
        ...
        if ($postData['isdraft'] == 'true') {
            ...
            foreach ($rows as $key => $row) {
                ...
                $this->db->where(array('id' =>  $get_id['id'], 'cKode_for' =>  $kode_for, 'lDeleted' => 0/*, 'cKode_for_log' => */));
