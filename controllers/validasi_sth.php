...
class validasi_sth extends MX_Controller { 
    ...
    function index($action = '') {
        ...
        $grid->addList('cPic','mExt','iValidate','mValidate','dSubmit_validasi','iSubmit_validasi'); 
        $grid->setSortBy('id');
        $grid->setSortOrder('DESC');  

        $grid->addFields('cPic','mExt','iValidate','mValidate','dSubmit_validasi', 'form_history'); 

        $grid->setWidth('cPic', '350');
        $grid->setAlign('cPic', 'left');
        $grid->setLabel('cPic', 'PIC');

        $grid->setWidth('mExt', '250');
        $grid->setAlign('mExt', 'left');
        $grid->setLabel('mExt', 'Ext');
        ...
        $grid->setWidth('form_history', '110');
        $grid->setAlign('form_history', 'center');
        ...
        // join tabel
        // $grid->setJoinTable('purchasing.poir_header', 'poir_header.cPONumber = biflow_sth.vKodeRef', 'inner');
        ...
        $grid->setQuery('biflow_sth.lDeleted = 0 ', null);

        $this->cKode = $this->input->get('cKode');            
        $grid->setInputGet('cKode', $this->cKode);            
        $grid->setQuery('biflow_sth.cKode', $this->input->get('cKode'));
        ...
        $grid->setRequired('cPic','mExt','iValidate');
        ...
        switch ($action) {
            ...
            case 'createproses':
                ...
                // get cKode_sth
                $sql = "SELECT a.cKode_sth
                        FROM hrd.biflow_sth a
                        WHERE a.id = '".$lastId."' ";
                $row = $this->db->query($sql)->row_array();
                $cKode_sth = $row['cKode_sth'];
                        ...
                        foreach ($_FILES['upload_dok_upload_file']["error"] as $key => $error) {
                            if ($error == UPLOAD_ERR_OK) {
                                ...
                                if (move_uploaded_file($tmp_name, $file_path)) {
                                    $datainsert                         = array();
                                    $datainsert['cKode_sth']            = $cKode_sth;
                                    ...
                                    if ($this->db->insert('hrd.biflow_sth_file', $datainsert)){
                                    } else {
            ...
            case 'updateproses':
                ...
                if($isUpload){
                    ...
                    if (isset($_FILES['upload_dok_upload_file'])) {
                        ...
                        $this->db->where(array('cKode_sth' => $cKode_sth, 'iDeleted' => 0));
                        $this->db->update('hrd.biflow_sth_file', $update);
    ...
    function searchBox_validasi_sth_vKodeRef($rowData, $id) {
    ...
    function listBox_validasi_sth_upload_dok($value, $pk, $name, $rowData) {
        $rowData = get_object_vars($rowData);

        $sql = "SELECT *
                FROM hrd.biflow_sth_file a
                WHERE a.cKode_sth = '".$rowData['cKode_sth']."'
                AND a.iDeleted = 0
                ORDER BY a.id DESC ";
        $row = $this->db->query($sql)->row_array();
    ...
    function listBox_validasi_sth_validasi($value, $pk, $name, $rowData){
        $sql = "SELECT COUNT(case when a.iStatus_bi = 1 then 1 else null END) AS jumlah_no
                ...
                FROM hrd.biflow_sth_param a
                WHERE a.lDeleted = 0
                AND a.cKode_sth = '".$rowData->cKode_sth."' ";
        $row = $this->db->query($sql)->row_array();
    ...
    function listBox_validasi_sth_iValidate($value, $pk, $name, $rowData){
    ...
    function listBox_validasi_sth_cPic($value, $pk, $name, $rowData){
    ...
    function listBox_validasi_sth_del($value, $pk, $name, $rowData){
        // print_r($this->input->get());exit;
    ...
    function insertBox_validasi_sth_cPic($field, $id) {
        $o = '<input type="hidden" id="' . $id . '" name="' . $field . '" class="required" value="">';
        $o .= '<input type="text" id="' . $id . '_id" name="' . $field . '_id" value="" size="45">';
    ...
    function updateBox_validasi_sth_cPic($field, $id, $value, $rowData) {
        $sql = "SELECT CONCAT_WS(' - ', a.cNip, a.vName) AS showshow
                FROM hrd.employee a
                WHERE a.lDeleted = 0
                AND a.cNip = '" . $value . "' ";
        ...
        $o = '<input type="hidden" id="' . $id . '" name="' . $field . '" class="required" value="' . $value . '">';
        $o .= '<input type="text" id="' . $id . '_id" name="' . $field . '_id" value="' . $value_dis . '" size="45" readonly>';
    ...
    function insertBox_validasi_sth_mExt($field, $id) {
    ...
    function updateBox_validasi_sth_mExt($field, $id, $value, $rowData) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<input type="hidden" name="isdraft" id="isdraft">';
        $o .= '<div class="col-sm-6">';
        $o .= '<textarea id="'.$id.'" name="'.$field.'" class="ckeditorField form-control required-ckeditor" colspan="2" readonly>'.nl2br($value).'</textarea>';
        $o .= '<script>
                    $(function () {
                        CKEDITOR.replace(' . $id . ',{
                            height: "300px",
    ...
    function insertBox_validasi_sth_upload_dok($field, $id)
    {   
    ...
    function updateBox_validasi_sth_upload_dok($field, $id, $value, $rowData)
    {   
        // get data file
        $sql = "SELECT *
                FROM hrd.biflow_sth_file a
        ...
        $o = $linknya;
        $o .= '<input type="file" id="'.$id.'" name="'.$field.'_upload_file[]" class="input_rows1 '.$required.' fileupload multi multifile" size="15" >';
        $o .= '<input type="hidden" name="cKode_sth" value="'.$rowData['cKode_sth'].'">';

        return $o;
    }

    function insertBox_validasi_sth_validasi($field, $id) {
        $data['table']      = "hrd.biflow_sth_param";
        $data['table_key']  = "cKode_sth";
        $data['id']         = $id;
        $data['field']      = $field;
        $data['url']        = $this->url;
    ...
    function updateBox_validasi_sth_validasi($field, $id, $value, $rowData) {
        $data['table']      = "hrd.biflow_sth_param";
        $data['table_key']  = "cKode_sth";
        $data['id']         = $id;
        $data['field']      = $field;
    ...
    function updateBox_validasi_sth_iValidate($field, $id, $value, $rowData) {
    ...
    function updateBox_validasi_sth_mValidate($field, $id, $value, $rowData) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<div class="col-sm-6">';
    ...
    function insertBox_validasi_sth_dSubmit_validasi($field, $id) {
        $o = '<b>Auto after submit</b>';

        return $o;
    }
    
    function updateBox_validasi_sth_dSubmit_validasi($field, $id, $value, $rowData) {
        $o = '<b>Auto after submit</b>';
    ...
    function updateBox_validasi_sth_mFeedback($field, $id, $value, $rowData){

        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<input type="hidden" name="isdraft" id="isdraft">';
        $o .= '<textarea id="'.$id.'" name="'.$field.'" style="width: 650px; height: 100px;" size="250" maxlength ="250" colspan="2" readonly>'.nl2br($value).'</textarea>';

        return $o;
    }

    function updateBox_validasi_sth_form_history($field, $id, $value, $rowData)
    {
        $data['id'] = $id;
        $data['field'] = $field;
        ...
        $sql = 'SELECT sth.cPic, sth.mExt, sth.dSubmit, sth.mFeedback, sth.mValidate,
                    if(sth.iValidate = 1, "Valid", if(sth.iValidate = 2, "Not Valid", "Need To Be Validate")) AS Validasi,
                    if(sth.iSubmit = 1 AND sth.lDeleted = 0, "Submitted", IF (sth.lDeleted = 1, "Deleted", "Need To Be Submit")) AS Status
                FROM hrd.biflow_sth_log sth
                WHERE sth.cKode_sth = "' . $rowData['cKode_sth'] . '"
                AND sth.iSubmit NOT IN (0)
                ORDER BY sth.dSubmit ASC
        ';

        $datasql = $this->db->query($sql)->result_array();
        $data['datasql'] = $datasql;

        $o = $this->load->view('partial/modul/bi_flow_sth_history', $data, true);
        return $o;
    }
    ...
    function before_insert_processor($row, $postData) {
        ...
        $postData['cKode_sth']  = $this->generateCodeDetail($postData['cKode']);
    ...
    function before_update_processor($row, $postData) {
        ...
        $id = $postData['validasi_sth_id'];
        $ket = $postData['mValidate'];
        $stat = $postData['iValidate'];
        ...
        $kode_sth = $datanya['cKode_sth'];

        $sql = "SELECT * 
                FROM biflow_sth_log
                WHERE cKode_sth =  '".$kode_sth."'
                ORDER BY id DESC
                LIMIT 1
            ";
        ...
        if ($postData['isdraft'] == 'true') {
            ...
            foreach ($rows as $key => $row) {
                ...
                foreach ($row as $k => $r) {
                    ...
                    // $update['cKode_sth_log'] = $cKode_log;
                    $update[$k] = $r;
                    $update['iSubmit_validasi'] = 0;
                    $update['mValidate'] = $ket;
                    $update['iValidate'] = $stat;
                    $update['id'] = $get_id['id'];

                }

                $this->db->where(array('id' =>  $get_id['id'], 'cKode_sth' =>  $kode_sth, 'lDeleted' => 0/*, 'cKode_sth_log' => */));
                $this->db->update('hrd.biflow_sth_log', $update);
            }
    ...
    function after_insert_processor($fields, $id, $postData){
        ...
        //     $insBi['cKode_sth']         = $postData['cKode_sth'];
        ...
        //     if($this->db->insert('hrd.biflow_sth_param', $insBi)){
        //     }else{
        //         echo $this->db->last_query();
        //     }
        // }
    ...
    function after_update_processor($fields, $id, $postData) {
        ...
        // $this->db->where('cKode_sth', $postData['cKode_sth']);
        // $this->db->update('hrd.biflow_sth_param', $upDel);
    ...
    function generateCodeDetail($kodeHeader){
        ...
        if(!empty($row)){
            $nilai = str_replace($kodeHeader.'-', "", $row['cKode_sth']);
            $int = intval($nilai);
            if ($int > 0) {
                $nilai = $int + 1;
            } else {
    ...
    function generateCodeDetail_2($kodeHeader){
        $sql = "SELECT a.cKode_sth_i
                FROM hrd.biflow_sth_item a
                WHERE a.lDeleted = 0
                AND a.cKode_sth = '".$kodeHeader."'
                ORDER BY a.id DESC
                LIMIT 1 ";
        $row = $this->db->query($sql)->row_array();

        if(!empty($row)){
            $nilai = str_replace($kodeHeader.'-', "", $row['cKode_sth_i']);
            $int = intval($nilai);
            if ($int > 0) {
                $nilai = $int + 1;
            } else {
