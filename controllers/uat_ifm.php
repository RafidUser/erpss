...
class uat_ifm extends MX_Controller { 
    function __construct() {
        ...
        $this->title = 'Informasi Input Field Master';
        $this->url = 'uat_ifm';
        $this->urlH = 'bi_flow';        
    ...
    function index($action = '') {
        ...
        // join tabel
        // $grid->setJoinTable('purchasing.poir_header', 'poir_header.cPONumber = biflow_ifm.vKodeRef', 'inner');
        ...
        $grid->setQuery('biflow_ifm.lDeleted = 0 ', null);

        $this->cKode = $this->input->get('cKode');            
        $grid->setInputGet('cKode', $this->cKode);            
        $grid->setQuery('biflow_ifm.cKode', $this->input->get('cKode'));
        ...
        switch ($action) {
            ...
            case 'createproses':
                ...
                // get cKode_ifm
                $sql = "SELECT a.cKode_ifm
                        FROM hrd.biflow_ifm a
                        WHERE a.id = '".$lastId."' ";
                $row = $this->db->query($sql)->row_array();
                $cKode_ifm = $row['cKode_ifm'];

                $cKode_ifm_log = $cKode_ifm . '-HIST-' . $nowDate;
                        ...
                        foreach ($_FILES['upload_dok_upload_file']["error"] as $key => $error) {
                            if ($error == UPLOAD_ERR_OK) {
                                ...
                                if (move_uploaded_file($tmp_name, $file_path)) {
                                    $datainsert                         = array();
                                    $datainsert['cKode_ifm']            = $cKode_ifm;
                                    ...
                                    if (($this->db->insert('hrd.biflow_ifm_file', $datainsert)) && ($this->db->insert('hrd.biflow_ifm_file_log', $insertdataa))){
                                    } else {
            ...
            case 'updateproses':
                ...
                if($isUpload){
                    ...
                    if (isset($_FILES['upload_dok_upload_file'])) {
                        ...
                        $this->db->where(array('cKode_ifm' => $cKode_ifm, 'iDeleted' => 0));
                        $this->db->update('hrd.biflow_ifm_file', $update);

                        $up['iDeleted']             = 0;
                        $up['dUpdate']              = date('Y-m-d H:i:s');
                        $up['cUpdate']              = $this->user->gNIP;

                        $this->db->where(array('id' => $get_id['id'], 'cKode_ifm' => $cKode_ifm, 'iDeleted' => 0));
                        $this->db->update('hrd.biflow_ifm_file_log', $up);
    ...
    function searchBox_uat_ifm_vKodeRef($rowData, $id) {
    ...
    function listBox_uat_ifm_upload_dok($value, $pk, $name, $rowData) {
        $rowData = get_object_vars($rowData);

        $sql = "SELECT *
                FROM hrd.biflow_ifm_file a
                WHERE a.cKode_ifm = '".$rowData['cKode_ifm']."'
                AND a.iDeleted = 0
                ORDER BY a.id DESC ";
        $row = $this->db->query($sql)->row_array();
    ...
    function listBox_uat_ifm_validasi($value, $pk, $name, $rowData){
        $sql = "SELECT COUNT(case when a.iStatus_bi = 1 then 1 else null END) AS jumlah_no
                ...
                FROM hrd.biflow_ifm_param a
                WHERE a.lDeleted = 0
                AND a.cKode_ifm = '".$rowData->cKode_ifm."' ";
        $row = $this->db->query($sql)->row_array();
    ...
    function listBox_uat_ifm_iValidate_uat($value, $pk, $name, $rowData){
    ...
    function listBox_uat_ifm_del($value, $pk, $name, $rowData){
        // print_r($this->input->get());exit;
    ...
    function insertBox_uat_ifm_mKeterangan($field, $id) {
    ...
    function updateBox_uat_ifm_mKeterangan($field, $id, $value, $rowData) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<input type="hidden" name="isdraft" id="isdraft">';
        $o .= '<div class="col-sm-6">';
    ...
    function insertBox_uat_ifm_mValidate_uat($field, $id) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<div class="col-sm-6">';
        $o .= '<textarea id="'.$id.'" name="'.$field.'" class="ckeditorField form-control" colspan="2"></textarea>';
        $o .= '<script>
    ...
    function updateBox_uat_ifm_mValidate_uat($field, $id, $value, $rowData) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<div class="col-sm-6">';
    ...
    function insertBox_uat_ifm_iValidate_uat($field, $id, $value, $rowData) {
    ...
    function updateBox_uat_ifm_iValidate_uat($field, $id, $value, $rowData) {
        $array = array( 0   => '-- Belum Dipilih --',
                        1   => 'Valid',
                        2   => 'Not Valid');
    ...
    function insertBox_uat_ifm_upload_dok($field, $id)
    {   
    ...
    function updateBox_uat_ifm_vModul($field, $id, $value, $rowData)
    {   
    ...
    function updateBox_uat_ifm_upload_dok($field, $id, $value, $rowData)
    {   
        // get data file
        $sql = "SELECT *
                FROM hrd.biflow_ifm_file a
        ...
        $o = $linknya;
        // $o .= '<input type="file" id="'.$id.'" name="'.$field.'_upload_file[]" class="input_rows1 '.$required.' fileupload multi multifile" size="15" >';
        $o .= '<input type="hidden" name="cKode_ifm" value="'.$rowData['cKode_ifm'].'">';

        return $o;
    }
    
    function updateBox_uat_ifm_parameter($field, $id, $value, $rowData) {
        $data['table']      = "hrd.biflow_ifm_param";
        $data['table_key']  = "cKode_ifm";
        $data['id']         = $id;
        $data['field']      = $field;
        $data['url']        = $this->url;
    ...
    function insertBox_uat_ifm_validasi($field, $id) {
        $data['table']      = "hrd.biflow_ifm_param";
        $data['table_key']  = "cKode_ifm";
        $data['id']         = $id;
        $data['field']      = $field;
    ...
    function updateBox_uat_ifm_validasi($field, $id, $value, $rowData) {
        $data['table']      = "hrd.biflow_ifm_param";
        $data['table_key']  = "cKode_ifm";
        $data['id']         = $id;
        $data['field']      = $field;
    ...
    function insertBox_uat_ifm_dSubmit_uat($field, $id) {
        $o = '<b>Auto after submit</b>';

        return $o;
    }
    
    function updateBox_uat_ifm_dSubmit_uat($field, $id, $value, $rowData) {
        $o = '<b>Auto after submit</b>';
    ...
    function insertBox_uat_ifm_mFeedback($field, $id) {
    ...
    function updateBox_uat_ifm_mFeedback($field, $id, $value, $rowData) {
        // print_r($this->input->get());exit;
        $data = $this->input->get();
        $id = $data['id'];
    ...
    function insertBox_uat_ifm_form_history($field, $id)
    {
        $o = '<b> - </b>';
    ...
    function updateBox_uat_ifm_form_history($field, $id, $value, $rowData)
    {
        $data['id'] = $id;
        $data['field'] = $field;
        ...
        $sql = 'SELECT ifm.mKeterangan, ifm.dSubmit_uat,ff.vFilename, ifm.mValidate, ifm.mFeedback, ff.vFilename_generate,
                    if(ifm.iValidate = 1, "Valid", if(ifm.iValidate = 2, "Not Valid", "Need To Be Validate")) AS Validasi,
                    if(ifm.iSubmit_uat = 1 AND ifm.lDeleted = 0, "Submitted", IF (ifm.lDeleted = 1, "Deleted", "Need To Be Submit")) AS Status
                FROM hrd.biflow_ifm_log ifm
                LEFT JOIN hrd.biflow_ifm_file_log ff ON ff.cKode_ifm = ifm.cKode_ifm 
                WHERE ifm.cKode_ifm = "' . $rowData['cKode_ifm'] . '"
                AND ifm.iSubmit_uat NOT IN (0)
                GROUP BY ifm.id
                ORDER BY ifm.dSubmit_uat ASC
        ';
    ...
    function before_insert_processor($row, $postData) {
        ...
        $postData['cKode_ifm']  = $this->generateCodeDetail($postData['cKode']);
        ...
        if ($postData['isdraft'] == 'true') {
            ...
            foreach ($rows as $key => $row) {
                ...
                foreach ($row as $k => $r) {
                    ...
                    $insert['cKode_ifm_log'] = $cKode_log;
                    $insert[$k] = $r;
                ...
                unset($insert['id']);
                $this->db->insert('hrd.biflow_ifm_log', $insert);
            ...
            foreach ($rows as $key => $row) {
                ...
                foreach ($row as $k => $r) {
                    // $insert[$keyHist] = $kodeHist;
                    $update['cKode_ifm_log'] = $cKode_log;
                    $update[$k] = $r;
                ...
                // echo print_r($get_id);
                // exit;
                $this->db->insert('hrd.biflow_ifm_log', $update);
                // $this->db->where(array('id' =>  $get_id['id'], 'cKode_ifm' =>  $kode_ifm, 'lDeleted' => 0/*, 'cKode_br_log' => */));
                // $this->db->update('hrd.biflow_ifm_log', $update);
                // echo print_r($update);
                // exit;
            }
        }
    ...
    function after_insert_processor($fields, $id, $postData){
        ...
        foreach ($status_bi as $key => $bi) {
            ...
            $insBi['cKode_ifm']         = $postData['cKode_ifm'];
            ...
            if($this->db->insert('hrd.biflow_ifm_param', $insBi)){
            }else{
                echo $this->db->last_query();
            }
        }*/
    ...
    function after_update_processor($fields, $id, $postData) {
        ...
        $this->db->where('cKode_ifm', $postData['cKode_ifm']);
        $this->db->update('hrd.biflow_ifm_param', $upDel);
    ...
    function manipulate_update_button($buttons, $rowData){      
        ...
        if(array_intersect($arrDeptAssigned, $arrTeam)){
            ...
            $buttons['update_back'] = '<span style="color:red;" title="'.$arrDeptAssigned.'">You\'re Dept not Authorized</span>';
        }
    ...
    function generateCodeDetail($kodeHeader){
        ...
        if(!empty($row)){
            $nilai = str_replace($kodeHeader.'-', "", $row['cKode_ifm']);
            $int = intval($nilai);
            if ($int > 0) {
                $nilai = $int + 1;
            } else {
    ...
    function generateCodeDetail_2($kodeHeader){
        $sql = "SELECT a.cKode_ifm_i
                FROM hrd.biflow_ifm_item a
                WHERE a.lDeleted = 0
                AND a.cKode_ifm = '".$kodeHeader."'
                ORDER BY a.id DESC
                LIMIT 1 ";
        $row = $this->db->query($sql)->row_array();

        if(!empty($row)){
            $nilai = str_replace($kodeHeader.'-', "", $row['cKode_ifm_i']);
            $int = intval($nilai);
            if ($int > 0) {
                $nilai = $int + 1;
            } else {
