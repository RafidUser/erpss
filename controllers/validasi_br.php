...
class validasi_br extends MX_Controller { 
    function __construct() {
        ...
        $this->title = 'Business Requirement';
        $this->url = 'validasi_br';
        $this->urlH = 'bi_flow';        
    ...
    function index($action = '') {
        ...
        $grid->addList('mKeterangan','iValidate','mValidate','dSubmit_validasi','iSubmit_validasi');
        $grid->setSortBy('id');
        $grid->setSortOrder('DESC');

        $grid->addFields('mKeterangan','iValidate','mValidate','dSubmit_validasi', 'mFeedback', 'form_history'); 
        ...
        switch ($action) {
            ...
            case 'createproses':
                ...
                $sql = "SELECT a.cKode_br
                        FROM hrd.biflow_br a
                        WHERE a.id = '".$lastId."' ";
                $row = $this->db->query($sql)->row_array();
                $cKode_br = $row['cKode_br'];
                        ...
                        foreach ($_FILES['upload_dok_upload_file']["error"] as $key => $error) {
                            if ($error == UPLOAD_ERR_OK) {
                                ...
                                if (move_uploaded_file($tmp_name, $file_path)) {
                                    ...
                                    if ($this->db->insert('hrd.biflow_br_file', $datainsert)){
                                    } else {
            ...
            case 'updateproses':
                ...
                if($isUpload){
                    ...
                    if (isset($_FILES['upload_dok_upload_file'])) {
                        ...
                        $this->db->where(array('cKode_br' => $cKode_br, 'iDeleted' => 0));
                        $this->db->update('hrd.biflow_br_file', $update);
    ...
    function searchBox_validasi_br_vKodeRef($rowData, $id) {
    ...
    function listBox_validasi_br_upload_dok($value, $pk, $name, $rowData) {
        $rowData = get_object_vars($rowData);

        $sql = "SELECT *
                FROM hrd.biflow_br_file a
                WHERE a.cKode_br = '".$rowData['cKode_br']."'
                AND a.iDeleted = 0
                ORDER BY a.id DESC ";
        $row = $this->db->query($sql)->row_array();
        ...
        if (!empty($row)) {
            if(file_exists($row['vFilename_generate'])) {
                ...
                $linknya = '<a class="" href="javascript:;" onclick="window.location=\''.$link.'\'">'.$row['vFilename'].'</a> <br><br>';
            }else{
    ...
    function listBox_validasi_br_mKeterangan($value, $pk, $name, $rowData) {
        $limited = mb_strimwidth($value, 0, 60, "...");
        $cleared = preg_replace('/\s+/', ' ', trim($limited));

        return $cleared;
    }

    function listBox_validasi_br_mValidate($value, $pk, $name, $rowData) {
        $limited = mb_strimwidth($value, 0, 60, "...");
        $cleared = preg_replace('/\s+/', ' ', trim($limited));
    ...
    function listBox_validasi_br_validasi($value, $pk, $name, $rowData){
        $sql = "SELECT COUNT(case when a.iStatus_bi = 1 then 1 else null END) AS jumlah_no
                ...
                FROM hrd.biflow_br_param a
                WHERE a.lDeleted = 0
                AND a.cKode_br = '".$rowData->cKode_br."' ";
        $row = $this->db->query($sql)->row_array();
    ...
    function listBox_validasi_br_iValidate($value, $pk, $name, $rowData){
    ...
    function listBox_validasi_br_del($value, $pk, $name, $rowData){
        // print_r($this->input->get());exit;
    ...
    function insertBox_validasi_br_mKeterangan($field, $id) {
    ...
    function updateBox_validasi_br_mKeterangan($field, $id, $value, $rowData) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<input type="hidden" name="isdraft" id="isdraft">';
        $o .= '<div class="col-sm-6">';
    ...
    function insertBox_validasi_br_mValidate($field, $id) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<div class="col-sm-6">';
    ...
    function updateBox_validasi_br_mValidate($field, $id, $value, $rowData) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<div class="col-sm-6">';
    ...
    function updateBox_validasi_br_iValidate($field, $id, $value, $rowData) {
    ...
    function insertBox_validasi_br_upload_dok($field, $id)
    {   
    ...
    function updateBox_validasi_br_vModul($field, $id, $value, $rowData)
    {   
    ...
    function updateBox_validasi_br_upload_dok($field, $id, $value, $rowData)
    {   
        // get data file
        $sql = "SELECT *
                FROM hrd.biflow_br_file a
        ...
        $o = $linknya;
        // $o .= '<input type="file" id="'.$id.'" name="'.$field.'_upload_file[]" class="input_rows1 '.$required.' fileupload multi multifile" size="15" >';
        $o .= '<input type="hidden" name="cKode_br" value="'.$rowData['cKode_br'].'">';

        return $o;
    }
    
    function updateBox_validasi_br_parameter($field, $id, $value, $rowData) {
        $data['table']      = "hrd.biflow_br_param";
        $data['table_key']  = "cKode_br";
        $data['id']         = $id;
        $data['field']      = $field;
        $data['url']        = $this->url;
    ...
    function insertBox_validasi_br_validasi($field, $id) {
        $data['table']      = "hrd.biflow_br_param";
        $data['table_key']  = "cKode_br";
        $data['id']         = $id;
        $data['field']      = $field;
    ...
    function updateBox_validasi_br_validasi($field, $id, $value, $rowData) {
        $data['table']      = "hrd.biflow_br_param";
        $data['table_key']  = "cKode_br";
        $data['id']         = $id;
        $data['field']      = $field;
    ...
    function insertBox_validasi_br_dSubmit_validasi($field, $id) {
        $o = '<b>Auto after submit</b>';

        return $o;
    }
    
    function updateBox_validasi_br_dSubmit_validasi($field, $id, $value, $rowData) {
        $o = '<b>Auto after submit</b>';
    ...
    function updateBox_validasi_br_mFeedback($field, $id, $value, $rowData){
    ...
    function updateBox_validasi_br_form_history($field, $id, $value, $rowData)
    {
        $data['id'] = $id;
        $data['field'] = $field;
        ...
        $sql = 'SELECT br.mKeterangan, br.dSubmit, br.mFeedback, br.mValidate, 
                ...
                AND br.dSubmit NOT IN (0000-00-00)
        ...
        $o = $this->load->view('partial/modul/bi_flow_br_history', $data, true);
        return $o;
    }
    ...
    function after_insert_processor($fields, $id, $postData){
        ...
        //     $insBi['cKode_br']         = $postData['cKode_br'];
        ...
        //     if($this->db->insert('hrd.biflow_br_param', $insBi)){
        //     }else{
        //         echo $this->db->last_query();
        //     }
        // }
    ...
    function after_update_processor($fields, $id, $postData) {
        ...
        // $this->db->where('cKode_br', $postData['cKode_br']);
        // $this->db->update('hrd.biflow_br_param', $upDel);
