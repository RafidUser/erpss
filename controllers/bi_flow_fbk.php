...
class bi_flow_fbk extends MX_Controller { 
    function __construct() {
        ...
        $this->title = 'Feedback BI';
        $this->url = 'bi_flow_fbk';
        $this->urlH = 'bi_flow';        
    ...
    function index($action = '') {
        ...
        $grid->setLabel('mKeterangan', 'Keterangan');
        $grid->setLabel('validasi', 'Validasi');
        $grid->setLabel('tgl_submit', 'Tgl Upload');
    ...
    function searchBox_bi_flow_fbk_vKodeRef($rowData, $id) {
    ...
    function listBox_bi_flow_fbk_upload_dok($value, $pk, $name, $rowData) {
    ...
    function listBox_bi_flow_fbk_validasi($value, $pk, $name, $rowData){
    ...
    function listBox_bi_flow_fbk_cSubmit_fbk($value, $pk, $name, $rowData) {
    ...
    function listBox_bi_flow_fbk_tgl_submit($value, $pk, $name, $rowData) {
    ...
    function listBox_bi_flow_fbk_del($value, $pk, $name, $rowData){
        // print_r($this->input->get());exit;
        ...
        if($cekApprove['iApprove_validasi'] == 2 || $datanya['iSubmit'] == 1 || $datanya['cSubmit_fbk'] == 2){
            $delete = '';
        }else{
    ...
    function listBox_Action($row, $actions){
        ...
        // Cek Submit Header
        $sqlCek     = "SELECT a.iSubmit, a.cSubmit_fbk
        ...
        $submit_by = (!empty($cekSubmit))?$cekSubmit['cSubmit_fbk']:1;
        ...
        if ($cekApprove['iApprove_validasi'] == 2 || $submitFisik == 1 || $submit_by == 2 ){
    ...
    public function manipulate_grid_button($button){
        ...
        $sqlCek     = "SELECT a.iSubmit_validasi, a.cSubmit_fbk
                        FROM ".$this->main_table." a
                        WHERE a.".$this->main_table_pk." = '".$row[$this->main_table_pk]."' ";
        $cekSub  = $this->db->query($sqlCek)->row_array();

        $cekSubmit  = $this->db->get_where($this->header_table, array('cKode' => $this->input->get('cKode')))->row_array();

        if ( $cekSubmit['iApprove_validasi'] == 1 ){
            array_unshift($button, $btn_baru);
        } else {
            unset($button['create']);
            if($cekSubmit['iApprove_validasi'] == 0){
                $button['create'] = 'Silahkan Submit Terlebih Dahulu';
            } else {
                $button['create'] = 'Sudah Approve';
            }
        }
    ...
    function insertBox_bi_flow_fbk_mKeterangan($field, $id) {
    ...
    function updateBox_bi_flow_fbk_mKeterangan($field, $id, $value, $rowData) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<input type="hidden" name="isdraft" id="isdraft">';
    ...
    function insertBox_bi_flow_fbk_upload_dok($field, $id)
    {   
    ...
    function updateBox_bi_flow_fbk_upload_dok($field, $id, $value, $rowData)
    {   
        // get data file
        $sql = "SELECT *
                FROM hrd.biflow_fbk_file a
    ...
    function insertBox_bi_flow_fbk_validasi($field, $id) {
    ...
    function updateBox_bi_flow_fbk_validasi($field, $id, $value, $rowData) {
        $data['table']      = "hrd.biflow_fbk_param";
        $data['table_key']  = "cKode_fbk";
        $data['id']         = $id;
        $data['field']      = $field;
    ...
    function insertBox_bi_flow_fbk_tgl_submit($field, $id) {
        $o = '<b>Auto after submit</b>';

        return $o;
    }
    
    function updateBox_bi_flow_fbk_tgl_submit($field, $id, $value, $rowData) {
        $o = '<b>Auto after submit</b>';
    ...
    function insertBox_bi_flow_fbk_cSubmit_fbk($field, $id, $value, $rowData) {
        $o = '<b>Auto after submit</b>';
    ...
    function UpdateBox_bi_flow_fbk_cSubmit_fbk($field, $id, $value, $rowData) {
    ...
    function insertBox_bi_flow_fbk_form_history($field, $id)
    {
        $o = '<b> - </b>';
    ...
    function updateBox_bi_flow_fbk_form_history($field, $id, $value, $rowData)
    {
        $data['id'] = $id;
        $data['field'] = $field;
