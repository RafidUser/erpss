...
class bi_flow_sop extends MX_Controller { 
    ...
    function index($action = '') {
        ...
        switch ($action) {
            ...
            case 'createproses':
                ...
                $cKode_sop_log = $cKode_sop . '-HIST-' . $nowDate;
                        ...
                        foreach ($_FILES['upload_dok_upload_file']["error"] as $key => $error) {
                            if ($error == UPLOAD_ERR_OK) {
                                ...
                                if (move_uploaded_file($tmp_name, $file_path)) {
                                    ...
                                    $insertdataa                         = array();
                                    $insertdataa['cKode_sop']            = $cKode_sop;
                                    $insertdataa['cKode_sop_log']        = $cKode_sop_log;
                                    ...
                                    if (($this->db->insert('hrd.biflow_sop_file', $datainsert)) && ($this->db->insert('hrd.biflow_sop_file_log', $insertdataa))){
                                    } else {
            ...
            case 'updateproses':
                ...
                if($isUpload){
                    ...
                    if (isset($_FILES['upload_dok_upload_file'])) {
                        ...
                        $this->db->where(array('id' => $get_id['id'], 'cKode_sop' => $cKode_sop, 'iDeleted' => 0));
                        $this->db->update('hrd.biflow_sop_file_log', $up);
            ...
            case 'delete':
                $postData = $this->input->post();
                    ...
                    $this->db->where(array('id' => $get_id['id'], 'cKode_sop' => $kode_sop));
                    $this->db->update('hrd.biflow_sop_log', $up);
                    // echo print_r($kode_sop);exit;
    ...
    function searchBox_bi_flow_sop_vKodeRef($rowData, $id) {
    ...
    function listBox_bi_flow_sop_upload_dok($value, $pk, $name, $rowData) {
    ...
    function listBox_bi_flow_sop_validasi($value, $pk, $name, $rowData){
    ...
    function listBox_bi_flow_sop_del($value, $pk, $name, $rowData){
        // print_r($this->input->get());exit;
    ...
    function insertBox_bi_flow_sop_mKeterangan($field, $id) {
    ...
    function updateBox_bi_flow_sop_mKeterangan($field, $id, $value, $rowData) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<input type="hidden" name="isdraft" id="isdraft">';
        $o .= '<div class="col-sm-6">';
    ...
    function insertBox_bi_flow_sop_upload_dok($field, $id)
    {   
    ...
    function updateBox_bi_flow_sop_upload_dok($field, $id, $value, $rowData)
    {   
        // get data file
        $sql = "SELECT *
                FROM hrd.biflow_sop_file a
    ...
    function insertBox_bi_flow_sop_parameter($field, $id) {
    ...
    function updateBox_bi_flow_sop_parameter($field, $id, $value, $rowData) {
        $data['table']      = "hrd.biflow_sop_param";
        $data['table_key']  = "cKode_sop";
        $data['id']         = $id;
        $data['field']      = $field;
    ...
    function insertBox_bi_flow_sop_dSubmit($field, $id) {
        $o = '<b>Auto after submit</b>';

        return $o;
    }
    
    function updateBox_bi_flow_sop_dSubmit($field, $id, $value, $rowData) {
        $o = '<b>Auto after submit</b>';
    ...
    function insertBox_bi_flow_sop_mFeedback($field, $id) {
    ...
    function updateBox_bi_flow_sop_mFeedback($field, $id, $value, $rowData) {
        // print_r($this->input->get());exit;
        $data = $this->input->get();
        $id = $data['id'];
    ...
    function insertBox_bi_flow_sop_form_history($field, $id)
    {
        $o = '<b> - </b>';
    ...
    function updateBox_bi_flow_sop_form_history($field, $id, $value, $rowData)
    {
        $data['id'] = $id;
        $data['field'] = $field;
        ...
        if ($postData['isdraft'] == 'true') {
            ...
            foreach ($rows as $key => $row) {
                ...
                foreach ($row as $k => $r) {
                    ...
                    $insert['cKode_sop_log'] = $cKode_log;
                    $insert[$k] = $r;
                ...
                unset($insert['id']);
                $this->db->insert('hrd.biflow_sop_log', $insert);
            ...
            foreach ($rows as $key => $row) {
                ...
                // echo print_r($get_id);
                // exit;
                $this->db->insert('hrd.biflow_sop_log', $update);
                // $this->db->where(array('id' =>  $get_id['id'], 'cKode_sop' =>  $kode_sop, 'lDeleted' => 0/*, 'cKode_br_log' => */));
