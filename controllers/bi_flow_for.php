...
class bi_flow_for extends MX_Controller { 
    function __construct() {
        ...
        $this->title = 'Rumus Perhitungan Modul';
        $this->url = 'bi_flow_for';
        $this->urlH = 'bi_flow';        
    ...
    function index($action = '') {
        ...
        $grid->addList('del','upload_dok','mKeterangan','dSubmit','iValidate','iSubmit'); 
        $grid->setSortBy('id');
        ...
        // join tabel
        // $grid->setJoinTable('purchasing.poir_header', 'poir_header.cPONumber = biflow_for.vKodeRef', 'inner');
        ...
        $grid->setQuery('biflow_for.lDeleted = 0 ', null);

        $this->cKode = $this->input->get('cKode');            
        $grid->setInputGet('cKode', $this->cKode);            
        $grid->setQuery('biflow_for.cKode', $this->input->get('cKode'));
        ...
        switch ($action) {
            ...
            case 'createproses':
                ...
                // get cKode_for
                $sql = "SELECT a.cKode_for
                        FROM hrd.biflow_for a
                        WHERE a.id = '".$lastId."' ";
                $row = $this->db->query($sql)->row_array();
                $cKode_for = $row['cKode_for'];

                $cKode_for_log = $cKode_for . '-HIST-' . $nowDate;
                        ...
                        foreach ($_FILES['upload_dok_upload_file']["error"] as $key => $error) {
                            if ($error == UPLOAD_ERR_OK) {
                                ...
                                if (move_uploaded_file($tmp_name, $file_path)) {
                                    $datainsert                         = array();
                                    $datainsert['cKode_for']            = $cKode_for;
                                    ...
                                    $insertdataa                         = array();
                                    $insertdataa['cKode_for']            = $cKode_for;
                                    $insertdataa['cKode_for_log']        = $cKode_for_log;
                                    ...
                                    if (($this->db->insert('hrd.biflow_for_file', $datainsert)) && ($this->db->insert('hrd.biflow_for_file_log', $insertdataa))){
                                    } else {
            ...
            case 'updateproses':
                ...
                if($isUpload){
                    ...
                    if (isset($_FILES['upload_dok_upload_file'])) {
                        ...
                        $this->db->where(array('cKode_for' => $cKode_for, 'iDeleted' => 0));
                        $this->db->update('hrd.biflow_for_file', $update);

                        $up['iDeleted']             = 0;
                        $up['dUpdate']              = date('Y-m-d H:i:s');
                        $up['cUpdate']              = $this->user->gNIP;

                        $this->db->where(array('id' => $get_id['id'], 'cKode_for' => $cKode_for, 'iDeleted' => 0));
                        $this->db->update('hrd.biflow_for_file_log', $up);
            ...
            case 'delete':
                $postData = $this->input->post();
                    ...
                    $sql2 = "SELECT * 
                                    FROM biflow_for_file_log
                                    WHERE cKode_for =  '".$kode_for."'
                                    ORDER BY id DESC
                                    LIMIT 1
                                ";
                    $get_file_id = $this->db->query($sql2)->row_array();
                    
                    $up['lDeleted'] = 1;
                    $upt['iDeleted'] = 1;

                    $this->db->where(array('id' => $get_id['id'], 'cKode_for' => $kode_for));
                    $this->db->update('hrd.biflow_for_log', $up);

                    $this->db->where(array('id' => $get_file_id['id'], 'cKode_for' => $kode_for));
                    $this->db->update('hrd.biflow_for_file_log', $upt);
                    // echo print_r($kode_for);exit;
    ...
    function searchBox_bi_flow_for_vKodeRef($rowData, $id) {
    ...
    function listBox_bi_flow_for_upload_dok($value, $pk, $name, $rowData) {
        $rowData = get_object_vars($rowData);

        $sql = "SELECT *
                FROM hrd.biflow_for_file a
                WHERE a.cKode_for = '".$rowData['cKode_for']."'
                AND a.iDeleted = 0
                ORDER BY a.id DESC ";
        $row = $this->db->query($sql)->row_array();
    ...
    function listBox_bi_flow_for_validasi($value, $pk, $name, $rowData){
        $sql = "SELECT COUNT(case when a.iStatus_bi = 1 then 1 else null END) AS jumlah_no
                ...
                FROM hrd.biflow_for_param a
                WHERE a.lDeleted = 0
                AND a.cKode_for = '".$rowData->cKode_for."' ";
        $row = $this->db->query($sql)->row_array();
    ...
    function listBox_bi_flow_for_del($value, $pk, $name, $rowData){
        // print_r($this->input->get());exit;
    ...
    function insertBox_bi_flow_for_mKeterangan($field, $id) {
    ...
    function updateBox_bi_flow_for_mKeterangan($field, $id, $value, $rowData) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<input type="hidden" name="isdraft" id="isdraft">';
        $o .= '<div class="col-sm-6">';
    ...
    function insertBox_bi_flow_for_upload_dok($field, $id)
    {   
    ...
    function updateBox_bi_flow_for_upload_dok($field, $id, $value, $rowData)
    {   
        // get data file
        $sql = "SELECT *
                FROM hrd.biflow_for_file a
        ...
        $o = $linknya;
        $o .= '<input type="file" id="'.$id.'" name="'.$field.'_upload_file[]" class="input_rows1 fileupload multi multifile" size="15" >';
        $o .= '<input type="hidden" name="cKode_for" value="'.$rowData['cKode_for'].'">';

        return $o;
    }

    function insertBox_bi_flow_for_parameter($field, $id) {
        $data['table']      = "hrd.biflow_for_param";
        $data['table_key']  = "cKode_for";
        $data['id']         = $id;
        $data['field']      = $field;
        $data['url']        = $this->url;
    ...
    function updateBox_bi_flow_for_parameter($field, $id, $value, $rowData) {
        $data['table']      = "hrd.biflow_for_param";
        $data['table_key']  = "cKode_for";
        $data['id']         = $id;
        $data['field']      = $field;
    ...
    function insertBox_bi_flow_for_dSubmit($field, $id) {
        $o = '<b>Auto after submit</b>';

        return $o;
    }
    
    function updateBox_bi_flow_for_dSubmit($field, $id, $value, $rowData) {
        $o = '<b>Auto after submit</b>';
    ...
    function insertBox_bi_flow_for_mFeedback($field, $id) {
    ...
    function updateBox_bi_flow_for_mFeedback($field, $id, $value, $rowData) {
        // print_r($this->input->get());exit;
        $data = $this->input->get();
        $id = $data['id'];
    ...
    function insertBox_bi_flow_for_form_history($field, $id)
    {
        $o = '<b> - </b>';
    ...
    function updateBox_bi_flow_for_form_history($field, $id, $value, $rowData)
    {
        $data['id'] = $id;
        $data['field'] = $field;
        ...
        $sql = 'SELECT fr.mKeterangan, fr.dSubmit, ff.vFilename, fr.mValidate, fr.mFeedback, ff.vFilename_generate 
                    ,if(fr.iValidate = 1, "Valid", if(fr.iValidate = 2, "Not Valid", "Need To Be Validate")) AS Validasi,
                    -- COALESCE(if(fr.iSubmit = 1 AND fr.lDeleted = 0, "Submitted", IF (fr.lDeleted = 1, "Deleted", "Need To Be Submit")) ,
                    -- if(ff.iDeleted = 0, "Submitted", IF (ff.iDeleted = 1, "Deleted", "Need To Be Submit")) ) AS Status
                    if(fr.iSubmit = 1 AND fr.lDeleted = 0, "Submitted", IF (fr.lDeleted = 1, "Deleted", "Need To Be Submit")) AS Status
                FROM hrd.biflow_for_log fr
                LEFT JOIN hrd.biflow_for_file_log ff ON ff.cKode_for = fr.cKode_for 
                WHERE fr.cKode_for = "' . $rowData['cKode_for'] . '"
                AND fr.iSubmit NOT IN (0)
                GROUP BY fr.id
                Order by fr.dSubmit ASC
        ';
    ...
    function before_insert_processor($row, $postData) {
        ...
        $postData['cKode_for']  = $this->generateCodeDetail($postData['cKode']);
        ...
        if ($postData['isdraft'] == 'true') {
            ...
            foreach ($rows as $key => $row) {
                ...
                foreach ($row as $k => $r) {
                    ...
                    $insert['cKode_for_log'] = $cKode_log;
                    $insert[$k] = $r;
                ...
                unset($insert['id']);
                $this->db->insert('hrd.biflow_for_log', $insert);
            ...
            foreach ($rows as $key => $row) {
                ...
                foreach ($row as $k => $r) {
                    // $insert[$keyHist] = $kodeHist;
                    $update['cKode_for_log'] = $cKode_log;
                    $update[$k] = $r;
                ...
                // echo print_r($get_id);
                // exit;
                $this->db->insert('hrd.biflow_for_log', $update);
                // $this->db->where(array('id' =>  $get_id['id'], 'cKode_for' =>  $kode_for, 'lDeleted' => 0/*, 'cKode_br_log' => */));
                // $this->db->update('hrd.biflow_for_log', $update);
                // echo print_r($update);
                // exit;
            }
        }
    ...
    function after_update_processor($fields, $id, $postData) {
        ...
        // $this->db->where('cKode_for', $postData['cKode_for']);
        // $this->db->update('hrd.biflow_for_param', $upDel);
        ...
        //     $insBi['cKode_for']         = $postData['cKode_for'];
        ...
        //     if($this->db->insert('hrd.biflow_for_param', $insBi)){
        //     }else{
        //         echo $this->db->last_query();
        //     }
        // }
    ...
    function generateCodeDetail($kodeHeader){
        ...
        if(!empty($row)){
            $nilai = str_replace($kodeHeader.'-', "", $row['cKode_for']);
            $int = intval($nilai);
            if ($int > 0) {
                $nilai = $int + 1;
            } else {
    ...
    function generateCodeDetail_2($kodeHeader){
        $sql = "SELECT a.cKode_for_i
                FROM hrd.biflow_for_item a
                WHERE a.lDeleted = 0
                AND a.cKode_for = '".$kodeHeader."'
                ORDER BY a.id DESC
                LIMIT 1 ";
        $row = $this->db->query($sql)->row_array();

        if(!empty($row)){
            $nilai = str_replace($kodeHeader.'-', "", $row['cKode_for_i']);
            $int = intval($nilai);
            if ($int > 0) {
                $nilai = $int + 1;
            } else {
