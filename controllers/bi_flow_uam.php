...
class bi_flow_uam extends MX_Controller { 
    ...
    function index($action = '') {
        ...
        switch ($action) {
            ...
            case 'createproses':
                ...
                $cKode_uam_log = $cKode . '-HIST-' . $nowDate;
                        ...
                        foreach ($_FILES['upload_dok_upload_file']["error"] as $key => $error) {
                            if ($error == UPLOAD_ERR_OK) {
                                ...
                                if (move_uploaded_file($tmp_name, $file_path)) {
                                    ...
                                    $insertdataa                         = array();
                                    $insertdataa['cKode_uam']            = $cKode_uam;
                                    $insertdataa['cKode_uam_log']        = $cKode_uam_log;
                                    ...
                                    if (($this->db->insert('hrd.biflow_uam_file', $datainsert)) && ($this->db->insert('hrd.biflow_uam_file_log', $insertdataa))){
                                    } else {
            ...
            case 'updateproses':
                ...
                $cKode_uam_log = $cKode_uam . '-HIST-' . $nowDate;
            ...
            case 'delete':
                $postData = $this->input->post();
                    ...
                    $this->db->where(array('id' => $get_id['id'], 'cKode_uam' => $kode_uam));
                    $this->db->update('hrd.biflow_uam_log', $up);
                    // echo print_r($kode_uam);exit;
    ...
    function searchBox_bi_flow_uam_vKodeRef($rowData, $id) {
    ...
    function listBox_bi_flow_uam_upload_dok($value, $pk, $name, $rowData) {
    ...
    function listBox_bi_flow_uam_validasi($value, $pk, $name, $rowData){
    ...
    function listBox_bi_flow_uam_del($value, $pk, $name, $rowData){
        // print_r($this->input->get());exit;
    ...
    function insertBox_bi_flow_uam_mKeterangan($field, $id) {
    ...
    function updateBox_bi_flow_uam_mKeterangan($field, $id, $value, $rowData) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<input type="hidden" name="isdraft" id="isdraft">';
        $o .= '<div class="col-sm-6">';
    ...
    function insertBox_bi_flow_uam_upload_dok($field, $id)
    {   
    ...
    function updateBox_bi_flow_uam_upload_dok($field, $id, $value, $rowData)
    {   
        // get data file
        $sql = "SELECT *
                FROM hrd.biflow_uam_file a
    ...
    function insertBox_bi_flow_uam_parameter($field, $id) {
    ...
    function updateBox_bi_flow_uam_parameter($field, $id, $value, $rowData) {
        $data['table']      = "hrd.biflow_uam_param";
        $data['table_key']  = "cKode_uam";
        $data['id']         = $id;
        $data['field']      = $field;
    ...
    function insertBox_bi_flow_uam_dSubmit($field, $id) {
        $o = '<b>Auto after submit</b>';

        return $o;
    }
    
    function updateBox_bi_flow_uam_dSubmit($field, $id, $value, $rowData) {
        $o = '<b>Auto after submit</b>';
    ...
    function insertBox_bi_flow_uam_mFeedback($field, $id) {
    ...
    function updateBox_bi_flow_uam_mFeedback($field, $id, $value, $rowData) {
        // print_r($this->input->get());exit;
        $data = $this->input->get();
        $id = $data['id'];
    ...
    function insertBox_bi_flow_uam_form_history($field, $id)
    {
        $o = '<b> - </b>';
    ...
    function updateBox_bi_flow_uam_form_history($field, $id, $value, $rowData)
    {
        $data['id'] = $id;
        $data['field'] = $field;
        ...
        if ($postData['isdraft'] == 'true') {
            ...
            foreach ($rows as $key => $row) {
                ...
                foreach ($row as $k => $r) {
                    ...
                    $insert['cKode_uam_log'] = $cKode_log;
                    $insert[$k] = $r;
                ...
                unset($insert['id']);
                $this->db->insert('hrd.biflow_uam_log', $insert);
            ...
            foreach ($rows as $key => $row) {
                ...
                // echo print_r($get_id);
                // exit;
                $this->db->insert('hrd.biflow_uam_log', $update);
                // $this->db->where(array('id' =>  $get_id['id'], 'cKode_uam' =>  $kode_uam, 'lDeleted' => 0/*, 'cKode_br_log' => */));
