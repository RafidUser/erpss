...
class validasi_uam extends MX_Controller { 
    function __construct() {
        ...
        $this->title = 'User Access Matrix';
        $this->url = 'validasi_uam';
        $this->urlH = 'bi_flow';        
    ...
    function index($action = '') {
        ...
        // join tabel
        // $grid->setJoinTable('purchasing.poir_header', 'poir_header.cPONumber = biflow_uam.vKodeRef', 'inner');
        ...
        $grid->setQuery('biflow_uam.lDeleted = 0 ', null);

        $this->cKode = $this->input->get('cKode');            
        $grid->setInputGet('cKode', $this->cKode);            
        $grid->setQuery('biflow_uam.cKode', $this->input->get('cKode'));
        ...
        switch ($action) {
            ...
            case 'createproses':
                ...
                // get cKode_uam
                $sql = "SELECT a.cKode_uam
                        FROM hrd.biflow_uam a
                        WHERE a.id = '".$lastId."' ";
                $row = $this->db->query($sql)->row_array();
                $cKode_uam = $row['cKode_uam'];
                        ...
                        foreach ($_FILES['upload_dok_upload_file']["error"] as $key => $error) {
                            if ($error == UPLOAD_ERR_OK) {
                                ...
                                if (move_uploaded_file($tmp_name, $file_path)) {
                                    $datainsert                         = array();
                                    $datainsert['cKode_uam']            = $cKode_uam;
                                    ...
                                    if ($this->db->insert('hrd.biflow_uam_file', $datainsert)){
                                    } else {
            ...
            case 'updateproses':
                ...
                if($isUpload){
                    ...
                    if (isset($_FILES['upload_dok_upload_file'])) {
                        ...
                        $this->db->where(array('cKode_uam' => $cKode_uam, 'iDeleted' => 0));
                        $this->db->update('hrd.biflow_uam_file', $update);
    ...
    function searchBox_validasi_uam_vKodeRef($rowData, $id) {
    ...
    function listBox_validasi_uam_upload_dok($value, $pk, $name, $rowData) {
        $rowData = get_object_vars($rowData);

        $sql = "SELECT *
                FROM hrd.biflow_uam_file a
                WHERE a.cKode_uam = '".$rowData['cKode_uam']."'
                AND a.iDeleted = 0
                ORDER BY a.id DESC ";
        $row = $this->db->query($sql)->row_array();
    ...
    function listBox_validasi_uam_validasi($value, $pk, $name, $rowData){
        $sql = "SELECT COUNT(case when a.iStatus_bi = 1 then 1 else null END) AS jumlah_no
                ...
                FROM hrd.biflow_uam_param a
                WHERE a.lDeleted = 0
                AND a.cKode_uam = '".$rowData->cKode_uam."' ";
        $row = $this->db->query($sql)->row_array();
    ...
    function listBox_validasi_uam_iValidate($value, $pk, $name, $rowData){
    ...
    function listBox_validasi_uam_del($value, $pk, $name, $rowData){
        // print_r($this->input->get());exit;
    ...
    function insertBox_validasi_uam_mKeterangan($field, $id) {
    ...
    function updateBox_validasi_uam_mKeterangan($field, $id, $value, $rowData) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<input type="hidden" name="isdraft" id="isdraft">';
        $o .= '<div class="col-sm-6">';
    ...
    function insertBox_validasi_uam_mValidate($field, $id) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<div class="col-sm-6">';
        $o .= '<textarea id="'.$id.'" name="'.$field.'" class="ckeditorField form-control" colspan="2"></textarea>';
        $o .= '<script>
    ...
    function updateBox_validasi_uam_mValidate($field, $id, $value, $rowData) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<div class="col-sm-6">';
    ...
    function updateBox_validasi_uam_iValidate($field, $id, $value, $rowData) {
    ...
    function insertBox_validasi_uam_upload_dok($field, $id)
    {   
    ...
    function updateBox_validasi_uam_vModul($field, $id, $value, $rowData)
    {   
    ...
    function updateBox_validasi_uam_upload_dok($field, $id, $value, $rowData)
    {   
        // get data file
        $sql = "SELECT *
                FROM hrd.biflow_uam_file a
        ...
        $o = $linknya;
        // $o .= '<input type="file" id="'.$id.'" name="'.$field.'_upload_file[]" class="input_rows1 '.$required.' fileupload multi multifile" size="15" >';
        $o .= '<input type="hidden" name="cKode_uam" value="'.$rowData['cKode_uam'].'">';

        return $o;
    }
    
    function updateBox_validasi_uam_parameter($field, $id, $value, $rowData) {
        $data['table']      = "hrd.biflow_uam_param";
        $data['table_key']  = "cKode_uam";
        $data['id']         = $id;
        $data['field']      = $field;
        $data['url']        = $this->url;
    ...
    function insertBox_validasi_uam_validasi($field, $id) {
        $data['table']      = "hrd.biflow_uam_param";
        $data['table_key']  = "cKode_uam";
        $data['id']         = $id;
        $data['field']      = $field;
    ...
    function updateBox_validasi_uam_validasi($field, $id, $value, $rowData) {
        $data['table']      = "hrd.biflow_uam_param";
        $data['table_key']  = "cKode_uam";
        $data['id']         = $id;
        $data['field']      = $field;
    ...
    function insertBox_validasi_uam_dSubmit_validasi($field, $id) {
        $o = '<b>Auto after submit</b>';

        return $o;
    }
    
    function updateBox_validasi_uam_dSubmit_validasi($field, $id, $value, $rowData) {
        $o = '<b>Auto after submit</b>';
    ...
    function updateBox_validasi_uam_mFeedback($field, $id, $value, $rowData){
    ...
    function updateBox_validasi_uam_form_history($field, $id, $value, $rowData)
    {
        $data['id'] = $id;
        $data['field'] = $field;
        ...
        $sql = 'SELECT uam.mKeterangan, uam.dSubmit, ff.vFilename,  uam.mFeedback, uam.mValidate, ff.vFilename_generate,
                    if(uam.iValidate = 1, "Valid", if(uam.iValidate = 2, "Not Valid", "Need To Be Validate")) AS Validasi,
                    if(uam.iSubmit = 1 AND uam.lDeleted = 0, "Submitted", IF (uam.lDeleted = 1, "Deleted", "Need To Be Submit")) AS Status
                FROM hrd.biflow_uam_log uam
                LEFT JOIN hrd.biflow_uam_file_log ff ON ff.cKode_uam = uam.cKode_uam
                WHERE uam.cKode_uam = "' . $rowData['cKode_uam'] . '"
                AND uam.iSubmit NOT IN (0)
                GROUP BY uam.id
                ORDER BY uam.dSubmit ASC
        ';

        $datasql = $this->db->query($sql)->result_array();
        $data['datasql'] = $datasql;

        $o = $this->load->view('partial/modul/bi_flow_uam_history', $data, true);
        return $o;
    }
    ...
    function before_insert_processor($row, $postData) {
        ...
        $postData['cKode_uam']  = $this->generateCodeDetail($postData['cKode']);
    ...
    function before_update_processor($row, $postData) {
        ...
        $kode_uam = $datanya['cKode_uam'];

        $sql = "SELECT * 
                FROM biflow_uam_log
                WHERE cKode_uam =  '".$kode_uam."'
                ORDER BY id DESC
                LIMIT 1
            ";
        ...
        if ($postData['isdraft'] == 'true') {
            ...
            foreach ($rows as $key => $row) {
                ...
                foreach ($row as $k => $r) {
                    ...
                    // $update['cKode_uam_log'] = $cKode_log;
                    $update[$k] = $r;
                ...
                $this->db->where(array('id' =>  $get_id['id'], 'cKode_uam' =>  $kode_uam, 'lDeleted' => 0/*, 'cKode_uam_log' => */));
                $this->db->update('hrd.biflow_uam_log', $update);
            }
    ...
    function after_insert_processor($fields, $id, $postData){
        ...
        foreach ($status_bi as $key => $bi) {
            ...
            $insBi['cKode_uam']         = $postData['cKode_uam'];
            ...
            if($this->db->insert('hrd.biflow_uam_param', $insBi)){
            }else{
                echo $this->db->last_query();
            }
        }*/
    ...
    function after_update_processor($fields, $id, $postData) {
        ...
        $this->db->where('cKode_uam', $postData['cKode_uam']);
        $this->db->update('hrd.biflow_uam_param', $upDel);
    ...
    function generateCodeDetail($kodeHeader){
        ...
        if(!empty($row)){
            $nilai = str_replace($kodeHeader.'-', "", $row['cKode_uam']);
            $int = intval($nilai);
            if ($int > 0) {
                $nilai = $int + 1;
            } else {
    ...
    function generateCodeDetail_2($kodeHeader){
        $sql = "SELECT a.cKode_uam_i
                FROM hrd.biflow_uam_item a
                WHERE a.lDeleted = 0
                AND a.cKode_uam = '".$kodeHeader."'
                ORDER BY a.id DESC
                LIMIT 1 ";
        $row = $this->db->query($sql)->row_array();

        if(!empty($row)){
            $nilai = str_replace($kodeHeader.'-', "", $row['cKode_uam_i']);
            $int = intval($nilai);
            if ($int > 0) {
                $nilai = $int + 1;
            } else {
