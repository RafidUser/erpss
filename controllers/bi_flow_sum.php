...
class bi_flow_sum extends MX_Controller { 
    function __construct() {
        ...
        $this->maintable = 'hrd.biflow_history';
    ...
    function index($action = '') {
        ...
        $grid->addList('del','cKode','iApprove', 'dApproval', 'cApproval');
        $grid->setSortBy('id');
        $grid->setSortOrder('DESC');  

        $grid->addFields('cKode', 'formary');

        $grid->setWidth('cKode', '110');
        $grid->setAlign('cKode', 'center');
        $grid->setLabel('cKode', 'Kode Generate');

        $grid->setWidth('iApprove', '150');
        $grid->setAlign('iApprove', 'center');
        $grid->setLabel('iApprove', 'Status Approval');

        $grid->setWidth('dApproval', '120');
        $grid->setAlign('dApproval', 'center');
        $grid->setLabel('dApproval', 'Tgl Approval');
        ...
        // join tabel
        // $grid->setJoinTable('purchasing.poir_header', 'poir_header.cPONumber = biflow_sum.vKodeRef', 'inner');
        ...
        $grid->setQuery('biflow_sum.lDeleted = 0 ', null);

        $this->cKode = $this->input->get('cKode');            
        $grid->setInputGet('cKode', $this->cKode);            
        $grid->setQuery('biflow_sum.cKode', $this->input->get('cKode'));
        ...
        switch ($action) {
            ...
            case 'createproses':
                ...
                // get cKode_sum
                $sql = "SELECT a.cKode_sum
                        FROM hrd.biflow_sum a
                        WHERE a.id = '".$lastId."' ";
                $row = $this->db->query($sql)->row_array();
                $cKode_sum = $row['cKode_sum'];
                        ...
                        foreach ($_FILES['upload_dok_upload_file']["error"] as $key => $error) {
                            if ($error == UPLOAD_ERR_OK) {
                                ...
                                if (move_uploaded_file($tmp_name, $file_path)) {
                                    $datainsert                         = array();
                                    $datainsert['cKode_sum']            = $cKode_sum;
                                    ...
                                    if ($this->db->insert('hrd.biflow_sum_file', $datainsert)){
                                    } else {
            ...
            case 'updateproses':
                ...
                if($isUpload){
                    ...
                    if (isset($_FILES['upload_dok_upload_file'])) {
                        ...
                        $this->db->where(array('cKode_sum' => $cKode_sum, 'iDeleted' => 0));
                        $this->db->update('hrd.biflow_sum_file', $update);
    ...
    function searchBox_bi_flow_sum_vKodeRef($rowData, $id) {
    ...
    function listBox_bi_flow_sum_upload_dok($value, $pk, $name, $rowData) {
        $rowData = get_object_vars($rowData);

        $sql = "SELECT *
                FROM hrd.biflow_sum_file a
                WHERE a.cKode_sum = '".$rowData['cKode_sum']."'
                AND a.iDeleted = 0
                ORDER BY a.id DESC ";
        $row = $this->db->query($sql)->row_array();
    ...
    function listBox_bi_flow_sum_validasi($value, $pk, $name, $rowData){
        $sql = "SELECT COUNT(case when a.iStatus_bi = 1 then 1 else null END) AS jumlah_no
                ...
                FROM hrd.biflow_sum_param a
                WHERE a.lDeleted = 0
                AND a.cKode_sum = '".$rowData->cKode_sum."' ";
        $row = $this->db->query($sql)->row_array();
    ...
    function listBox_bi_flow_sum_del($value, $pk, $name, $rowData){
        // print_r($this->input->get());exit;
    ...
    function insertBox_bi_flow_sum_mKeterangan($field, $id) {
    ...
    function updateBox_bi_flow_sum_mKeterangan($field, $id, $value, $rowData) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<div class="col-sm-6">';
    ...
    function insertBox_bi_flow_sum_upload_dok($field, $id)
    {   
    ...
    function updateBox_bi_flow_sum_upload_dok($field, $id, $value, $rowData)
    {   
        // get data file
        $sql = "SELECT *
                FROM hrd.biflow_sum_file a
        ...
        $o = $linknya;
        $o .= '<input type="file" id="'.$id.'" name="'.$field.'_upload_file[]" class="input_rows1 '.$required.' fileupload multi multifile" size="15" >';
        $o .= '<input type="hidden" name="cKode_sum" value="'.$rowData['cKode_sum'].'">';

        return $o;
    }

    function insertBox_bi_flow_sum_validasi($field, $id) {
        $data['table']      = "hrd.biflow_sum_param";
        $data['table_key']  = "cKode_sum";
        $data['id']         = $id;
        $data['field']      = $field;
        $data['url']        = $this->url;
    ...
    function updateBox_bi_flow_sum_validasi($field, $id, $value, $rowData) {
        $data['table']      = "hrd.biflow_sum_param";
        $data['table_key']  = "cKode_sum";
        $data['id']         = $id;
        $data['field']      = $field;
    ...
    function before_insert_processor($row, $postData) {
        ...
        $postData['cKode_sum']  = $this->generateCodeDetail($postData['cKode']);
    ...
    function after_insert_processor($fields, $id, $postData){
        ...
        foreach ($status_bi as $key => $bi) {
            ...
            $insBi['cKode_sum']         = $postData['cKode_sum'];
            ...
            if($this->db->insert('hrd.biflow_sum_param', $insBi)){
            }else{
                echo $this->db->last_query();
            }
        }
    ...
    function after_update_processor($fields, $id, $postData) {
        ...
        $this->db->where('cKode_sum', $postData['cKode_sum']);
        $this->db->update('hrd.biflow_sum_param', $upDel);
    ...
    function manipulate_insert_button($buttons) { 
       ...
       /*  $save_draft = '<button type="button"
        ...
        $save = '<button type="button"
                        ...
                        id="button_create_'.$this->url.'"
                        class="icon-save ui-button" 
                        onclick="javascript:save_btn_'.$this->url.'(\''.$this->url.'\', \' '.base_url().'processor/'.$this->urlpath.'?draft=false&cKode='.$this->input->get('foreign_key').'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\', this, false )">Save</button>';

        $buttons['save_back'] = $save.$js;
...
/*         $update_draft = '<button type="button"
        ...
        $update = '<button type="button"
                        ...
                        id="button_update_draft_'.$this->url.'"
                        class="ui-button-text icon-save"
                        onclick="javascript:update_btn_'.$this->url.'(\''.$this->url.'\', \' '.base_url().'processor/'.$this->urlpath.'?draft=false&cKode='.$this->input->get('foreign_key').'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\', this, false)">Update</button>';

        $buttons['update_back'] = $update.$js;
    ...
    function generateCodeDetail($kodeHeader){
        ...
        if(!empty($row)){
            $nilai = str_replace($kodeHeader.'-', "", $row['cKode_sum']);
            $int = intval($nilai);
            if ($int > 0) {
                $nilai = $int + 1;
            } else {
    ...
    function generateCodeDetail_2($kodeHeader){
        $sql = "SELECT a.cKode_sum_i
                FROM hrd.biflow_sum_item a
                WHERE a.lDeleted = 0
                AND a.cKode_sum = '".$kodeHeader."'
                ORDER BY a.id DESC
                LIMIT 1 ";
        $row = $this->db->query($sql)->row_array();

        if(!empty($row)){
            $nilai = str_replace($kodeHeader.'-', "", $row['cKode_sum_i']);
            $int = intval($nilai);
            if ($int > 0) {
                $nilai = $int + 1;
            } else {
