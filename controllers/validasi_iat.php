...
class validasi_iat extends MX_Controller { 
    ...
    function index($action = '') {
        ...
        $grid->addList('upload_dok','vModul','mKeterangan','iValidate','mValidate','dSubmit_validasi','iSubmit_validasi');
        $grid->setSortBy('id');
        $grid->setSortOrder('DESC');

        // $grid->addFields('upload_dok','vModul','mKeterangan','iValidate','mValidate','parameter','dSubmit_validasi'); 
        $grid->addFields('upload_dok','vModul','mKeterangan','iValidate','mValidate','dSubmit_validasi', 'mFeedback','form_history'); 
        ...
        $grid->setRequired('vModul','iValidate'); 
                        ...
                        foreach ($_FILES['upload_dok_upload_file']["error"] as $key => $error) {
                            if ($error == UPLOAD_ERR_OK) {
                                ...
                                if (move_uploaded_file($tmp_name, $file_path)) {
                                    ...
                                    if ($this->db->insert('hrd.biflow_iat_file', $datainsert)){
                                    } else {
    ...
    function searchBox_validasi_iat_vKodeRef($rowData, $id) {
    ...
    function listBox_validasi_iat_upload_dok($value, $pk, $name, $rowData) {
    ...
    function listBox_validasi_iat_validasi($value, $pk, $name, $rowData){
    ...
    function listBox_validasi_iat_iValidate($value, $pk, $name, $rowData){
    ...
    function listBox_validasi_iat_del($value, $pk, $name, $rowData){
        // print_r($this->input->get());exit;
    ...
    function insertBox_validasi_iat_mKeterangan($field, $id) {
    ...
    function updateBox_validasi_iat_mKeterangan($field, $id, $value, $rowData) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<input type="hidden" name="isdraft" id="isdraft">';
        $o .= '<div class="col-sm-6">';
    ...
    function insertBox_validasi_iat_mValidate($field, $id) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<div class="col-sm-6">';
        $o .= '<textarea id="'.$id.'" name="'.$field.'" class="ckeditorField form-control" colspan="2"></textarea>';
        $o .= '<script>
    ...
    function updateBox_validasi_iat_mValidate($field, $id, $value, $rowData) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<div class="col-sm-6">';
    ...
    function updateBox_validasi_iat_iValidate($field, $id, $value, $rowData) {
    ...
    function insertBox_validasi_iat_upload_dok($field, $id)
    {   
    ...
    function updateBox_validasi_iat_vModul($field, $id, $value, $rowData)
    {   
    ...
    function updateBox_validasi_iat_upload_dok($field, $id, $value, $rowData)
    {   
        // get data file
        $sql = "SELECT *
                FROM hrd.biflow_iat_file a
    ...
    function updateBox_validasi_iat_parameter($field, $id, $value, $rowData) {
    ...
    function insertBox_validasi_iat_validasi($field, $id) {
        $data['table']      = "hrd.biflow_iat_param";
        $data['table_key']  = "cKode_iat";
        $data['id']         = $id;
        $data['field']      = $field;
    ...
    function updateBox_validasi_iat_validasi($field, $id, $value, $rowData) {
        $data['table']      = "hrd.biflow_iat_param";
        $data['table_key']  = "cKode_iat";
        $data['id']         = $id;
        $data['field']      = $field;
    ...
    function insertBox_validasi_iat_dSubmit_validasi($field, $id) {
        $o = '<b>Auto after submit</b>';

        return $o;
    }
    
    function updateBox_validasi_iat_dSubmit_validasi($field, $id, $value, $rowData) {
        $o = '<b>Auto after submit</b>';
    ...
    function updateBox_validasi_iat_mFeedback($field, $id, $value, $rowData){
    ...
    function insertBox_validasi_iat_form_history($field, $id)
    {
        $o = '<b> - </b>';
    ...
    function updateBox_validasi_iat_form_history($field, $id, $value, $rowData)
    {
        $data['id'] = $id;
        $data['field'] = $field;
        ...
        $sql = 'SELECT iat.vModul, iat.mKeterangan, iat.dSubmit, ff.vFilename, iat.mFeedback, 
                        iat.mValidate, ff.vFilename_generate,
                    if(iat.iValidate = 1, "Valid", if(iat.iValidate = 2, "Not Valid", "Need To Be Validate")) AS Validasi,
                    if(iat.iSubmit = 1 AND iat.lDeleted = 0, "Submitted", IF (iat.lDeleted = 1, "Deleted", "Need To Be Submit")) AS Status
                ...
                AND iat.iSubmit NOT IN (0)
                GROUP BY iat.id
                ORDER BY iat.dSubmit ASC
        ';

        $datasql = $this->db->query($sql)->result_array();
        $data['datasql'] = $datasql;

        $o = $this->load->view('partial/modul/bi_flow_iat_history', $data, true);
        return $o;
    }
