...
class uat_ffu extends MX_Controller { 
    ...
    function index($action = '') {
        ...
        $grid->addFields('upload_dok','mKeterangan','dSubmit');
        ...
        // join tabel
        // $grid->setJoinTable('purchasing.poir_header', 'poir_header.cPONumber = biflow_ffu.vKodeRef', 'inner');
        ...
        $grid->setQuery('biflow_ffu.lDeleted = 0 ', null);

        $this->cKode = $this->input->get('cKode');            
        $grid->setInputGet('cKode', $this->cKode);            
        $grid->setQuery('biflow_ffu.cKode', $this->input->get('cKode'));
        ...
        switch ($action) {
            ...
            case 'createproses':
                ...
                // get cKode_ffu
                $sql = "SELECT a.cKode_ffu
                        FROM hrd.biflow_ffu a
                        WHERE a.id = '".$lastId."' ";
                $row = $this->db->query($sql)->row_array();
                $cKode_ffu = $row['cKode_ffu'];

                // $uat = "SELECT bi.raw_id 
                //         FROM hrd.biflow bi
                //         WHERE bi.cKode = '".$ckode."'
                //     ";
                // $qr = $this->db->query($uat)->row_array();
                // $id_raw = $qr['raw_id'];
                ...
                if($isUpload){
                    ...
                    $fpath  = $pathf . "/" . $lastId;
                    // $ppath  = "uploads/nplnet/ssuat/" . $id_raw;
                        ...
                        foreach ($_FILES['upload_dok_upload_file']["error"] as $key => $error) {
                            if ($error == UPLOAD_ERR_OK) {
                                ...
                                if (move_uploaded_file($tmp_name, $file_path)) {
                                    $datainsert                         = array();
                                    $datainsert['cKode_ffu']            = $cKode_ffu;
                                    ...
                                    if ($this->db->insert('hrd.biflow_ffu_file', $datainsert)){
                                    } else {
            ...
            case 'updateproses':
                ...
                if($isUpload){
                    ...
                    if (isset($_FILES['upload_dok_upload_file'])) {
                        ...
                        $this->db->where(array('cKode_ffu' => $cKode_ffu, 'iDeleted' => 0));
                        $this->db->update('hrd.biflow_ffu_file', $update);
    ...
    function searchBox_uat_ffu_vKodeRef($rowData, $id) {
    ...
    function listBox_uat_ffu_upload_dok($value, $pk, $name, $rowData) {
        $rowData = get_object_vars($rowData);

        $sql = "SELECT *
                FROM hrd.biflow_ffu_file a
                WHERE a.cKode_ffu = '".$rowData['cKode_ffu']."'
                AND a.iDeleted = 0
                ORDER BY a.id DESC ";
        $row = $this->db->query($sql)->row_array();
    ...
    function listBox_uat_ffu_validasi($value, $pk, $name, $rowData){
        $sql = "SELECT COUNT(case when a.iStatus_bi = 1 then 1 else null END) AS jumlah_no
                ...
                FROM hrd.biflow_ffu_param a
                WHERE a.lDeleted = 0
                AND a.cKode_ffu = '".$rowData->cKode_ffu."' ";
        $row = $this->db->query($sql)->row_array();
    ...
    function listBox_uat_ffu_del($value, $pk, $name, $rowData){
        // print_r($this->input->get());exit;
        ...
        if($dHead['iSubmit_uat'] == 1 || $datanya['iSubmit'] == 1){
            $delete = '';
        }else{
    ...
    function insertBox_uat_ffu_mKeterangan($field, $id) {
    ...
    function updateBox_uat_ffu_mKeterangan($field, $id, $value, $rowData) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<input type="hidden" name="isdraft" id="isdraft">';
        $o .= '<div class="col-sm-6">';
    ...
    function insertBox_uat_ffu_upload_dok($field, $id)
    {   
    ...
    function updateBox_uat_ffu_upload_dok($field, $id, $value, $rowData)
    {   
        // get data file
        $sql = "SELECT *
                FROM hrd.biflow_ffu_file a
        ...
        $o = $linknya;
        $o .= '<input type="file" id="'.$id.'" name="'.$field.'_upload_file[]" class="input_rows1 fileupload multi multifile" size="15" >';
        $o .= '<input type="hidden" name="cKode_ffu" value="'.$rowData['cKode_ffu'].'">';

        return $o;
    }

    function insertBox_uat_ffu_parameter($field, $id) {
        $data['table']      = "hrd.biflow_ffu_param";
        $data['table_key']  = "cKode_ffu";
        $data['id']         = $id;
        $data['field']      = $field;
        $data['url']        = $this->url;
    ...
    function updateBox_uat_ffu_parameter($field, $id, $value, $rowData) {
        $data['table']      = "hrd.biflow_ffu_param";
        $data['table_key']  = "cKode_ffu";
        $data['id']         = $id;
        $data['field']      = $field;
    ...
    function insertBox_uat_ffu_dSubmit($field, $id) {
        $o = '<b>Auto after submit</b>';

        return $o;
    }
    
    function updateBox_uat_ffu_dSubmit($field, $id, $value, $rowData) {
        $o = '<b>Auto after submit</b>';
    ...
    function before_insert_processor($row, $postData) {
        ...
        $postData['cKode_ffu']  = $this->generateCodeDetail($postData['cKode']);
    ...
    function after_insert_processor($fields, $id, $postData){
        ...
        //     $insBi['cKode_ffu']         = $postData['cKode_ffu'];
        ...
        //     if($this->db->insert('hrd.biflow_ffu_param', $insBi)){
        //     }else{
        //         echo $this->db->last_query();
        //     }
        // }
    ...
    function after_update_processor($fields, $id, $postData) {
        ...
        // $this->db->where('cKode_ffu', $postData['cKode_ffu']);
        // $this->db->update('hrd.biflow_ffu_param', $upDel);
    ...
    function generateCodeDetail($kodeHeader){
        $sql = "SELECT a.cKode_ffu
                ...
                ORDER BY CHAR_LENGTH(a.cKode_ffu) DESC, a.cKode_ffu DESC
                LIMIT 1 ";
        $row = $this->db->query($sql)->row_array();

        if(!empty($row)){
            $nilai = str_replace($kodeHeader.'-', "", $row['cKode_ffu']);
            $int = intval($nilai);
            if ($int > 0) {
                $nilai = $int + 1;
            } else {
