...
class uat_lpaw extends MX_Controller { 
    function __construct() {
        ...
        $this->title = 'Tabel Laporan Proses';
        $this->url = 'uat_lpaw';
        $this->urlH = 'bi_flow';        
        ...
        $this->maintable = 'hrd.biflow_lpaw';    
    ...
    function index($action = '') {
        ...
        $grid->addList('upload_dok','mKeterangan','iValidate_uat','mValidate_uat','dSubmit_uat','iSubmit_uat');
        $grid->setSortBy('id');
        $grid->setSortOrder('DESC');

        $grid->addFields('upload_dok','mKeterangan','iValidate_uat','mValidate_uat','dSubmit_uat', 'mFeedback', 'form_history'); 
        ...
        // join tabel
        // $grid->setJoinTable('purchasing.poir_header', 'poir_header.cPONumber = biflow_lpaw.vKodeRef', 'inner');
        ...
        $grid->setQuery('biflow_lpaw.lDeleted = 0 ', null);

        $this->cKode = $this->input->get('cKode');            
        $grid->setInputGet('cKode', $this->cKode);            
        $grid->setQuery('biflow_lpaw.cKode', $this->input->get('cKode'));
        ...
        switch ($action) {
            ...
            case 'createproses':
                ...
                // get cKode_lpaw
                $sql = "SELECT a.cKode_lpaw
                        FROM hrd.biflow_lpaw a
                        WHERE a.id = '".$lastId."' ";
                $row = $this->db->query($sql)->row_array();
                $cKode_lpaw = $row['cKode_lpaw'];

                $cKode_lpaw_log = $cKode_lpaw . '-HIST-' . $nowDate;
                        ...
                        foreach ($_FILES['upload_dok_upload_file']["error"] as $key => $error) {
                            if ($error == UPLOAD_ERR_OK) {
                                ...
                                if (move_uploaded_file($tmp_name, $file_path)) {
                                    $datainsert                         = array();
                                    $datainsert['cKode_lpaw']            = $cKode_lpaw;
                                    ...
                                    $insertdataa                         = array();
                                    $insertdataa['cKode_lpaw']           = $cKode_lpaw;
                                    $insertdataa['cKode_lpaw_log']       = $cKode_lpaw_log;
                                    ...
                                    if (($this->db->insert('hrd.biflow_lpaw_file', $datainsert)) && ($this->db->insert('hrd.biflow_lpaw_file_log', $insertdataa))){
                                    } else {
            ...
            case 'updateproses':
                ...
                if($isUpload){
                    ...
                    if (isset($_FILES['upload_dok_upload_file'])) {
                        ...
                        $sql = "SELECT * 
                                        FROM biflow_lpaw_log
                                        WHERE cKode_lpaw =  '".$cKode_lpaw."'
                                        ORDER BY id DESC
                                        LIMIT 1
                                    ";
                        ...
                        $this->db->where(array('cKode_lpaw' => $cKode_lpaw, 'iDeleted' => 0));
                        $this->db->update('hrd.biflow_lpaw_file', $update);

                        $up['iDeleted']             = 0;
                        $up['dUpdate']              = date('Y-m-d H:i:s');
                        $up['cUpdate']              = $this->user->gNIP;

                        $this->db->where(array('id' => $get_id['id'], 'cKode_lpaw' => $cKode_lpaw, 'iDeleted' => 0));
                        $this->db->update('hrd.biflow_lpaw_file_log', $up);
    ...
    function searchBox_uat_lpaw_vKodeRef($rowData, $id) {
    ...
    function listBox_uat_lpaw_upload_dok($value, $pk, $name, $rowData) {
        $rowData = get_object_vars($rowData);

        $sql = "SELECT *
                FROM hrd.biflow_lpaw_file a
                WHERE a.cKode_lpaw = '".$rowData['cKode_lpaw']."'
                AND a.iDeleted = 0
                ORDER BY a.id DESC ";
        $row = $this->db->query($sql)->row_array();
    ...
    function listBox_uat_lpaw_validasi($value, $pk, $name, $rowData){
        $sql = "SELECT COUNT(case when a.iStatus_bi = 1 then 1 else null END) AS jumlah_no
                ...
                FROM hrd.biflow_lpaw_param a
                WHERE a.lDeleted = 0
                AND a.cKode_lpaw = '".$rowData->cKode_lpaw."' ";
        $row = $this->db->query($sql)->row_array();
    ...
    function listBox_uat_lpaw_iValidate_uat($value, $pk, $name, $rowData){
    ...
    function listBox_uat_lpaw_del($value, $pk, $name, $rowData){
        // print_r($this->input->get());exit;
    ...
    function insertBox_uat_lpaw_mKeterangan($field, $id) {
    ...
    function updateBox_uat_lpaw_mKeterangan($field, $id, $value, $rowData) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<input type="hidden" name="isdraft" id="isdraft">';
        $o .= '<div class="col-sm-6">';
    ...
    function insertBox_uat_lpaw_mValidate_uat($field, $id) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<div class="col-sm-6">';
        $o .= '<textarea id="'.$id.'" name="'.$field.'" class="ckeditorField form-control" colspan="2"></textarea>';
        $o .= '<script>
    ...
    function updateBox_uat_lpaw_mValidate_uat($field, $id, $value, $rowData) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<div class="col-sm-6">';
    ...
    function insertBox_uat_lpaw_iValidate_uat($field, $id) {
    ...
    function updateBox_uat_lpaw_iValidate_uat($field, $id, $value, $rowData) {
        $array = array( 0   => '-- Belum Dipilih --',
                        1   => 'Valid',
                        2   => 'Not Valid');
    ...
    function insertBox_uat_lpaw_upload_dok($field, $id)
    {   
    ...
    function updateBox_uat_lpaw_vModul($field, $id, $value, $rowData)
    {   
    ...
    function updateBox_uat_lpaw_upload_dok($field, $id, $value, $rowData)
    {   
        // get data file
        $sql = "SELECT *
                FROM hrd.biflow_lpaw_file a
        ...
        $o = $linknya;
        // $o .= '<input type="file" id="'.$id.'" name="'.$field.'_upload_file[]" class="input_rows1 '.$required.' fileupload multi multifile" size="15" >';
        $o .= '<input type="hidden" name="cKode_lpaw" value="'.$rowData['cKode_lpaw'].'">';

        return $o;
    }
    
    function updateBox_uat_lpaw_parameter($field, $id, $value, $rowData) {
        $data['table']      = "hrd.biflow_lpaw_param";
        $data['table_key']  = "cKode_lpaw";
        $data['id']         = $id;
        $data['field']      = $field;
        $data['url']        = $this->url;
    ...
    function insertBox_uat_lpaw_validasi($field, $id) {
        $data['table']      = "hrd.biflow_lpaw_param";
        $data['table_key']  = "cKode_lpaw";
        $data['id']         = $id;
        $data['field']      = $field;
    ...
    function updateBox_uat_lpaw_validasi($field, $id, $value, $rowData) {
        $data['table']      = "hrd.biflow_lpaw_param";
        $data['table_key']  = "cKode_lpaw";
        $data['id']         = $id;
        $data['field']      = $field;
    ...
    function insertBox_uat_lpaw_dSubmit_uat($field, $id) {
        $o = '<b>Auto after submit</b>';

        return $o;
    }
    
    function updateBox_uat_lpaw_dSubmit_uat($field, $id, $value, $rowData) {
        $o = '<b>Auto after submit</b>';
    ...
    function insertBox_uat_lpaw_mFeedback($field, $id) {
    ...
    function updateBox_uat_lpaw_mFeedback($field, $id, $value, $rowData) {
        // print_r($this->input->get());exit;
        $data = $this->input->get();
        $id = $data['id'];
    ...
    function insertBox_uat_lpaw_form_history($field, $id)
    {
        $o = '<b> - </b>';
    ...
    function updateBox_uat_lpaw_form_history($field, $id, $value, $rowData)
    {
        $data['id'] = $id;
        $data['field'] = $field;
        ...
        $sql = 'SELECT lpaw.mKeterangan, lpaw.dSubmit_uat, ff.vFilename, lpaw.mFeedback, lpaw.mValidate, ff.vFilename_generate,
                    if(lpaw.iValidate = 1, "Valid", if(lpaw.iValidate = 2, "Not Valid", "Need To Be Validate")) AS Validasi,
                    if(lpaw.iSubmit_uat = 1 AND lpaw.lDeleted = 0, "Submitted", IF (lpaw.lDeleted = 1, "Deleted", "Need To Be Submit")) AS Status
                FROM hrd.biflow_lpaw_log lpaw
                LEFT JOIN hrd.biflow_lpaw_file_log ff ON ff.cKode_lpaw = lpaw.cKode_lpaw 
                WHERE lpaw.cKode_lpaw = "' . $rowData['cKode_lpaw'] . '"
                AND lpaw.iSubmit_uat NOT IN (0)
                GROUP BY lpaw.id
                ORDER BY lpaw.dSubmit_uat ASC
        ';
    ...
    function before_insert_processor($row, $postData) {
        ...
        $postData['cKode_lpaw']  = $this->generateCodeDetail($postData['cKode']);
        ...
        if ($postData['isdraft'] == 'true') {
            ...
            foreach ($rows as $key => $row) {
                ...
                foreach ($row as $k => $r) {
                    ...
                    $insert['cKode_lpaw_log'] = $cKode_log;
                    $insert[$k] = $r;
                ...
                unset($insert['id']);
                $this->db->insert('hrd.biflow_lpaw_log', $insert);
            ...
            foreach ($rows as $key => $row) {
                ...
                foreach ($row as $k => $r) {
                    // $insert[$keyHist] = $kodeHist;
                    $update['cKode_lpaw_log'] = $cKode_log;
                    $update[$k] = $r;
                ...
                // echo print_r($get_id);
                // exit;
                $this->db->insert('hrd.biflow_lpaw_log', $update);
                // $this->db->where(array('id' =>  $get_id['id'], 'cKode_lpaw' =>  $kode_lpaw, 'lDeleted' => 0/*, 'cKode_br_log' => */));
                // $this->db->update('hrd.biflow_lpaw_log', $update);
                // echo print_r($update);
                // exit;
            }
        }
    ...
    function after_insert_processor($fields, $id, $postData){
        ...
        foreach ($status_bi as $key => $bi) {
            ...
            $insBi['cKode_lpaw']         = $postData['cKode_lpaw'];
            ...
            if($this->db->insert('hrd.biflow_lpaw_param', $insBi)){
            }else{
                echo $this->db->last_query();
            }
        }*/
    ...
    function after_update_processor($fields, $id, $postData) {
        ...
        $this->db->where('cKode_lpaw', $postData['cKode_lpaw']);
        $this->db->update('hrd.biflow_lpaw_param', $upDel);
    ...
    function generateCodeDetail($kodeHeader){
        ...
        if(!empty($row)){
            $nilai = str_replace($kodeHeader.'-', "", $row['cKode_lpaw']);
            $int = intval($nilai);
            if ($int > 0) {
                $nilai = $int + 1;
            } else {
    ...
    function generateCodeDetail_2($kodeHeader){
        $sql = "SELECT a.cKode_lpaw_i
                FROM hrd.biflow_lpaw_item a
                WHERE a.lDeleted = 0
                AND a.cKode_lpaw = '".$kodeHeader."'
                ORDER BY a.id DESC
                LIMIT 1 ";
        $row = $this->db->query($sql)->row_array();

        if(!empty($row)){
            $nilai = str_replace($kodeHeader.'-', "", $row['cKode_lpaw_i']);
            $int = intval($nilai);
            if ($int > 0) {
                $nilai = $int + 1;
            } else {
