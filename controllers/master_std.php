<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
    class master_std  extends MX_Controller {
    private $sess_auth;
    private $dbset;
    ...
    function index($action = '') {
        ...
        $grid = new Grid;		
        $grid->setTitle('Standar Modul');		
        $grid->setTable('ss.std_ukuran_modul');		
        $grid->setUrl('master_std');		
        $grid->addList('vName','iMinRange','iMaxRange','lDeleted');
		
		$grid->setLabel('vName','Nama');		
		$grid->setLabel('iMinRange','Minimum Range');		
		$grid->setLabel('iMaxRange','Maximum Range');		
        ...
        $grid->addFields('vName','iMinRange','iMaxRange','cUpdatedBy','tCreated','tUpdated','lDeleted' );
		
		$grid->changeFieldType('lDeleted','combobox', '', array(''=>'-- All --', 0=>'Active', 1=>'Deleted'));
		
        $grid->setQuery('ss.std_ukuran_modul.lDeleted', 0);
	...
	function listBox_master_std_tCreated($value) {
		return date('d/m/Y H:i:s', strtotime($value));
	}
	function listBox_master_std_tUpdated($value) {
		return date('d M Y H:i:s', strtotime($value));
	}	
	public function insertBox_master_std_tCreated($field, $id) {
		return '<input type="hidden" value="'.date('Y-m-d H:i:s').'" name="'.$field.'" id="'.$id.'" />'.date('l, d F Y g:i:s A');
	}
	public function updateBox_master_std_tCreated($field, $id, $value) {
		return '<input type="hidden" value="'.date('Y-m-d H:i:s', strtotime($value)).'" name="'.$field.'" id="'.$id.'" />'.date('l, d F Y g:i:s A', strtotime($value));
		
	}	

	public function insertBox_master_std_tUpdated($field, $id) {
		return '<input type="hidden" value="'.date('Y-m-d H:i:s').'" name="'.$field.'" id="'.$id.'" />'.date('l, d F Y g:i:s A');
	}
	public function updateBox_master_std_tUpdated($field, $id, $value) {
		return '<input type="hidden" value="'.date('Y-m-d H:i:s', strtotime($value)).'" name="'.$field.'" id="'.$id.'" />'.date('l, d F Y g:i:s A', strtotime($value));
		
	}	
	
	function listBox_master_std_cUpdatedBy($value, $pk, $name, $rowData) {
	...
	public function insertBox_master_std_cUpdatedBy($field, $id) {
		$vName='';
		$cNip = $this->user->gNIP;
	...
	public function updateBox_master_std_cUpdatedBy($field, $id, $value,$rowData) {
