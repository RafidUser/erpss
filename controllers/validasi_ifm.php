...
class validasi_ifm extends MX_Controller { 
                        ...
                        foreach ($_FILES['upload_dok_upload_file']["error"] as $key => $error) {
                            if ($error == UPLOAD_ERR_OK) {
                                ...
                                if (move_uploaded_file($tmp_name, $file_path)) {
                                    ...
                                    if ($this->db->insert('hrd.biflow_ifm_file', $datainsert)){
                                    } else {
    ...
    function searchBox_validasi_ifm_vKodeRef($rowData, $id) {
    ...
    function listBox_validasi_ifm_upload_dok($value, $pk, $name, $rowData) {
    ...
    function listBox_validasi_ifm_validasi($value, $pk, $name, $rowData){
    ...
    function listBox_validasi_ifm_iValidate($value, $pk, $name, $rowData){
    ...
    function listBox_validasi_ifm_del($value, $pk, $name, $rowData){
        // print_r($this->input->get());exit;
    ...
    function insertBox_validasi_ifm_mKeterangan($field, $id) {
    ...
    function updateBox_validasi_ifm_mKeterangan($field, $id, $value, $rowData) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<input type="hidden" name="isdraft" id="isdraft">';
        $o .= '<div class="col-sm-6">';
    ...
    function insertBox_validasi_ifm_mValidate($field, $id) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<div class="col-sm-6">';
        $o .= '<textarea id="'.$id.'" name="'.$field.'" class="ckeditorField form-control" colspan="2"></textarea>';
        $o .= '<script>
    ...
    function updateBox_validasi_ifm_mValidate($field, $id, $value, $rowData) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<div class="col-sm-6">';
    ...
    function updateBox_validasi_ifm_iValidate($field, $id, $value, $rowData) {
    ...
    function insertBox_validasi_ifm_upload_dok($field, $id)
    {   
    ...
    function updateBox_validasi_ifm_vModul($field, $id, $value, $rowData)
    {   
    ...
    function updateBox_validasi_ifm_upload_dok($field, $id, $value, $rowData)
    {   
        // get data file
        $sql = "SELECT *
                FROM hrd.biflow_ifm_file a
    ...
    function updateBox_validasi_ifm_parameter($field, $id, $value, $rowData) {
    ...
    function insertBox_validasi_ifm_validasi($field, $id) {
        $data['table']      = "hrd.biflow_ifm_param";
        $data['table_key']  = "cKode_ifm";
        $data['id']         = $id;
        $data['field']      = $field;
    ...
    function updateBox_validasi_ifm_validasi($field, $id, $value, $rowData) {
        $data['table']      = "hrd.biflow_ifm_param";
        $data['table_key']  = "cKode_ifm";
        $data['id']         = $id;
        $data['field']      = $field;
    ...
    function insertBox_validasi_ifm_dSubmit_validasi($field, $id) {
        $o = '<b>Auto after submit</b>';

        return $o;
    }
    
    function updateBox_validasi_ifm_dSubmit_validasi($field, $id, $value, $rowData) {
        $o = '<b>Auto after submit</b>';
    ...
    function updateBox_validasi_ifm_mFeedback($field, $id, $value, $rowData){
    ...
    function updateBox_validasi_ifm_form_history($field, $id, $value, $rowData)
    {
        $data['id'] = $id;
        $data['field'] = $field;
        ...
        $sql = 'SELECT ifm.mKeterangan, ifm.dSubmit,ff.vFilename, ifm.mValidate, ifm.mFeedback, ff.vFilename_generate,
                    if(ifm.iValidate = 1, "Valid", if(ifm.iValidate = 2, "Not Valid", "Need To Be Validate")) AS Validasi,
                    if(ifm.iSubmit = 1 AND ifm.lDeleted = 0, "Submitted", IF (ifm.lDeleted = 1, "Deleted", "Need To Be Submit")) AS Status
                ...
                AND ifm.iSubmit NOT IN (0)
                GROUP BY ifm.id
                ORDER BY ifm.dSubmit ASC
        ';

        $datasql = $this->db->query($sql)->result_array();
        $data['datasql'] = $datasql;

        $o = $this->load->view('partial/modul/bi_flow_ifm_history', $data, true);
        return $o;
    }
    ...
    function before_update_processor($row, $postData) {
        ...
        if ($postData['isdraft'] == 'true') {
            ...
            foreach ($rows as $key => $row) {
                ...
                $this->db->where(array('id' =>  $get_id['id'], 'cKode_ifm' =>  $kode_ifm, 'lDeleted' => 0/*, 'cKode_ifm_log' => */));
