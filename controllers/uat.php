...
class uat extends MX_Controller
{
    public function __construct()
    {
        ...
        $datagrid['islist'] = array(
            ...
            'iSubmit_uat' => array('label' => 'Status Submit', 'width' => 150, 'align' => 'center', 'search' => true),
            'iApprove_uat' => array('label' => 'Status UAT', 'width' => 150, 'align' => 'center', 'search' => true),
        );

        $datagrid['setQuery'] = array(
            0 => array('vall' => 'biflow.lDeleted', 'nilai' => 0),
            1 => array('vall' => 'biflow.iSubmit', 'nilai' => 1),
            2 => array('vall' => 'biflow.iApprove_validasi', 'nilai' => 2),
        );

        $datagrid['jointableinner'] = array(
            0 => array('hrd.ss_raw_problems' => 'ss_raw_problems.id = biflow.raw_id AND YEAR(ss_raw_problems.date_posted) >= 2023'),
        );
    ...
    public function index($action = '')
    {
        ...
        $grid->changeFieldType('iSubmit_uat', 'combobox', ' ', array('' => '--Pilih--', 0 => 'Draft - Need to be Submit', 1 => 'Submited'));
        $grid->changeFieldType('iApprove_uat', 'combobox', '', array('' => '--Pilih--', 0 => 'Waiting Approval', 1 => 'Revise', 2 => 'Approved'));
        ...
        switch ($action) {
            ...
            /*release*/
            case 'release':
                echo $this->release_view();
                break;
            case 'release_process':
                echo $this->release_process();
                break;
            /*Release Final*/
            case 'release_final':
                echo $this->release_final_view();
                break;
            case 'release_final_process':
                echo $this->release_final_process();
                break;
            /*Confirm*/
            case 'approve':
    ...
    public function listBox_uat_cRequestor($value, $pk, $name, $rowData)
    {
        // get value
    ...
    public function listBox_uat_ss_raw_pic_pic($value, $pk, $name, $rowData)
    {
        // get value
        $sql = "SELECT CONCAT_WS(' - ', a.cNip, a.vName) AS showshow
                FROM hrd.employee a
    ...
    public function listBox_uat_itemas_c_teamc($value, $pk, $name, $rowData)
    {
    ...
    public function listBox_uat_iHet_ub($value, $pk, $name, $rowData)
    {
    ...
    public function listBox_uat_iHet_kp($value, $pk, $name, $rowData)
    {
        $r = number_format($value, 3, ',', '.');
    ...
    public function listBox_uat_itemas_n_scpri($value, $pk, $name, $rowData)
    {
        $r = number_format($value, 3, ',', '.');
    ...
    public function listBox_uat_iComp_mnf($value, $pk, $name, $rowData)
    {
    ...
    public function listBox_uat_upb_cTeam_pd($value, $pk, $name, $rowData)
    {
    ...
    public function searchBox_uat_ss_raw_pic_pic($rowData, $id)
    {
        // get nip PM
    ...
    public function searchBox_uat_cRequestor($rowData, $id)
    {
    ...
    public function insertBox_uat_form_detail($field, $id)
    {
        $get = $this->input->get();
        $post = $this->input->post();
    ...
    public function updateBox_uat_form_detail($field, $id, $value, $rowData)
    {
        $get = $this->input->get();
        $post = $this->input->post();
        foreach ($get as $kget => $vget) {
    ...
    public function approve_view($id = 0)
    {
        ...
        $echo .= 'Remark :
                ...
                <button type="button" id="bt_'.$this->url . '_approve" onclick="submit_ajax(\'form_' . $this->url . '_approve\')">' . $lblbutton . '</button>';
    ...
    public function approve_process()
    {
        ...
        $message = '';
        $arrValid = array(
            0 => array('name' => 'File UAT',
                'table' => 'hrd.biflow_fu',
                'key' => 'cKode_fu',
                'validate' => true),
            // 1 => array('name' => 'File Feedback UAT',
            //     'table' => 'hrd.biflow_ffu',
            //     'key' => 'cKode_ffu',
            //     'validate' => true),
        );
        // End Validasi
        ...
        if ($iapprove == 2) {
            ...
            $sql = 'select a.*,date(dclosePm) as dclosePm_short, date(tMarkedAsFinished) as tMarkedAsFinished_short, date(dcloseReq) as dcloseReq_sort
                    from hrd.ss_raw_problems a
                    where a.id in (' . $post['raw_id'] . ')
                    and a.iStatus in (6,13)
                    order by a.tMarkedAsFinished DESC limit 1 ';
            $dSSid = $this->db->query($sql)->row_array();

            if (!empty($dSSid)) {
                if ($dSSid['iStatus'] == 6) {
                    $now = $dSSid['dclosePm_short'];
                    $startdatei = strtotime("+ 1 days", strtotime($now));
                    $d2 = date("Y-m-d", $startdatei);
                } else {
                    $now = $dSSid['dcloseReq_sort'];
                    $startdatei = strtotime("+ 1 days", strtotime($now));
                    $d2 = date("Y-m-d", $startdatei);
                }

            } else {
                // belum finish ambil dateNow + 1
                $d2 = date('Y-m-d', strtotime(' +1 day'));
            }

            $dlastRaw = $this->getLastFinish($post['raw_id']);
            // print_r($dlastRaw);
            if (!empty($dlastRaw)) {
                // print_r($dlastRaw);
                $d1 = $dlastRaw['tMarkedAsFinished_short'];
                $dClosepm = $dlastRaw['dclosePm'];
            } else {
                $d1 = 'x';
            }

            $sql = 'select distinct(c.I_APP_ID),d.vURL
                    from hrd.ss_raw_problems_module a
                    join hrd.prv_appmodules b on b.ID=a.moduleId
                    join hrd.prv_application c on c.I_APP_ID=b.I_APP_ID
                    join hrd.svn_info d on d.iSvnId=c.iSvnId
                    where a.parentId in ("' . $post['raw_id'] . '")';
            // echo "<pre>".$sql;
            $dSvnUrl = $this->db->query($sql)->result_array();

            $dTags = '';

            if (!empty($dSvnUrl)) {
                $i = 1;
                foreach ($dSvnUrl as $dSvn) {
                    // print_r($d1);
                    //$return .=  trim($dSvn['vURL']);
                    // $return .= $i . '. ';
                    $dTags .= $this->getLastTags(trim($dSvn['vURL']), $d1, $d2);
                    $dTags .= '<br>';
                    // $dTags = $this->getLastTags(trim($dSvn['vURL']), $d1, $d2);
                    $i++;
                }
            }
            // print_r($dClosepm);exit;

            $insraw['iStatus'] = 13;
            $insraw['iCloseReq'] = 1;
            // $insraw['iCloseGm'] = 1;    
            $insraw['confirmPP'] = 2;    
            $insraw['cCloseReq'] = $cNip;
            $insraw['cCloseGm'] = 'N00319';
            $insraw['dCloseReq'] = $dClosepm;
            $insraw['dClosePm'] = $dClosepm;
            $insraw['dCloseGm'] = $dClosepm;
            // $insraw['dCloseReq'] = date('Y-m-d H:i:s');
            // $insraw['dCloseGm'] = date('Y-m-d H:i:s');

            $this->db->where('id', $post['raw_id']);
            $this->db->update('hrd.ss_raw_problems', $insraw);
            //print_r($this->db->last_query());
            // print_r($post['raw_id']); exit;

            // Proses Insert Parameter Selesai
            // get data Business Requirement
            $sqlBr = "SELECT *
                        FROM hrd.biflow_br br
                        WHERE br.lDeleted = 0
                        AND br.cKode = '".$post['cKode']."' ";
            $rowBr = $this->db->query($sqlBr)->result_array();

            // Update Raw Parameter yang kosong

            if(!empty($id_raw)){
                $up['lDeleted'] = 1;
                $up['tUpdated'] = date('Y-m-d H:i:s');
                $up['tUpdated_app'] = date('Y-m-d H:i:s');
                $up['cUpdated'] = $this->user->gNIP;
                $up['mUpdated'] = "erpss/controllers/uat.php :1134 (approve_process)";
                $this->db->where('id', $id_raw);
                $this->db->where('tParameter', "");
                $this->db->update('hrd.ss_raw_parameter', $up);

                foreach ($rowBr as $kbr => $vbr) {
                    $insBr['rawid'] = $post['raw_id'];
                    $insBr['tParameter'] = $vbr['mKeterangan'];
                    $insBr['iBobot'] = 100;
                    // $insraw['iCloseGm'] = 1;
                    // $insraw['iCloseReq'] = 1;

                    $this->db->insert('hrd.ss_raw_parameter', $insBr);
            ...
            // Update iSubmit (Sub Controller per kategori) menjadi 0
            foreach ($arrValid as $key => $valid) {
                $updSub['iSubmit'] = 0;
                $this->db->where(array('lDeleted' => 0, 'cKode' => $post['cKode']));
        ...
        // get data maintable
        $sqlH = "SELECT bi.*, rp.pic AS project_manager
                    FROM hrd.biflow bi
    ...
    public function release_view()
    {
        ...
        $echo .= '<h1>Release</h1><br />';
        $echo .= '<form id="form_' . $this->url . '_release" action="' . base_url() . 'processor/' . $this->urlpath . '?action=release_process" method="post">';
        $echo .= '<div style="vertical-align: top;">';
        $echo .= 'Remark :
        <input type="hidden" name="' . $this->main_table_pk . '" value="' . $this->input->get($this->main_table_pk) . '" />
        <input type="hidden" name="company_id" value="' . $this->input->get('company_id') . '" />
        ...
        <input type="hidden" name="raw_id" value="' . $this->input->get('raw_id') . '" />
                ...
                <textarea name="vRemark"></textarea>
        <button type="button" onclick="submit_ajax(\'form_' . $this->url . '_release\')">Submit</button>';
    ...
    public function release_process()
    {
        ...
        // Send Notifikasi
        $proses = 'Permintaan Release';
        // if($iapprove == '2'){
        //     $proses = 'Approve';
        // }else if($iapprove == '1'){
        //     $proses = 'Revise';
        // }
        $subject= $this->title . ' -> Permintaan Release';
        $content= 'Diberitahukan telah ada proses <b>'.$proses.'</b> ' . $this->title;
    ...
    public function release_final_view()
    {
        $echo = '<script type="text/javascript">
                     function submit_ajax(form_id) {
                        return $.ajax({
        ...
        $echo .= '<h1>Release</h1><br />';
        $echo .= '<form id="form_' . $this->url . '_release_final" action="' . base_url() . 'processor/' . $this->urlpath . '?action=release_final_process" method="post">';
        $echo .= '<div style="vertical-align: top;">';
        $echo .= 'Remark :
                    <input type="hidden" name="' . $this->main_table_pk . '" value="' . $this->input->get($this->main_table_pk) . '" />
                    <input type="hidden" name="company_id" value="' . $this->input->get('company_id') . '" />
                    ...
                    <textarea name="vRemark"></textarea>
                    <br/> Tgl Release :
                    <input type="date" size="8" name="dRelease" class="input_rows1 required tanggal" >
                    <button type="button" onclick="submit_ajax(\'form_' . $this->url . '_release_final\')">Release</button>';
    ...
    public function release_final_process()
    {
        $post = $this->input->post();
        $cNip = $this->user->gNIP;
        $vName = $this->user->gName;
        ...
        $updt['dRelease'] = $post['dRelease'];
        // print_r($post); 
        $this->db->where(array('cKode' => $post['cKode'], 'lDeleted' => 0));
        $this->db->update('hrd.biflow', $updt);
        ...
        //     $proses = 'Revise';
        // }
        $subject= $this->title . ' -> Release';
        $content= 'Diberitahukan telah ada proses <b>'.$proses.'</b> Project'/* . $this->title*/;
    ...
    public function before_update_processor($row, $postData)
    {
        ...
        if ($postData['isdraft'] == true) {
            ...
            $message = '';
            $arrValid = array(
                0 => array( 'name'          => 'Informasi Input Field Master', 
                            'table'         => 'hrd.biflow_ifm', 
                            'submit'        => true,
                            'submit_field'  => 'iSubmit_uat',),
                1 => array( 'name'          => 'Informasi Input Output', 
                            'table'         => 'hrd.biflow_iat', 
                            'submit'        => true,
                            'submit_field'  => 'iSubmit_uat',),
                2 => array( 'name'          => 'Tabel Laporan Proses', 
                            'table'         => 'hrd.biflow_lpaw', 
                            'submit'        => true, 
                            'submit_field'  => 'iSubmit_uat',),
                3 => array( 'name'          => 'File UAT', 
                            'table'         => 'hrd.biflow_fu', 
                            'submit'        => true, 
                            'submit_field'  => 'iSubmit',),
                // 4 => array( 'name'          => 'File Feedback UAT', 
                //             'table'         => 'hrd.biflow_ffu', 
                //             'submit'        => true, 
                //             'submit_field'  => 'iSubmit',),
            );
        ...
        if ($this->input->get('action') == 'view') {
            ...
            if ($isOpenEditing) {
                ...
                foreach ($activities as $act) {
                    $update_draft = '<button onclick="update_draft_btn(\'' . $this->url . '\', \' ' . base_url() . 'processor/' . $this->urlpath . '?draft=true&modul_id=' . $this->input->get('modul_id') . '&iM_modul_activity=' . $act['iM_modul_activity'] . ' \',this,true );" id="button_update_draft_' . $this->url . '" class="ui-button-text icon-save" >Update as Draft</button>';
                    ...
                    $release = '<button onclick="javascript:load_popup(\' ' . base_url() . 'processor/' . $this->urlpath . '?action=release&iM_modul_activity=' . $act['iM_modul_activity'] . '&iM_activity=' . $act['iM_activity'] . '&' . $this->main_table_pk . '=' . $peka . '&raw_id='.$raw_id.'&iupb_id=' . $iupb_id . '&cKode=' . $rowData['cKode'] . '&company_id=' . $this->input->get('company_id') . '&group_id=' . $this->input->get('group_id') . '&modul_id=' . $this->input->get('modul_id') . ' \')"  id="button_approve_"' . $this->url . '"  class="ui-button-text icon-save" >Release</button>';

                    $confirm_release = '<button onclick="javascript:load_popup(\' ' . base_url() . 'processor/' . $this->urlpath . '?action=release_final&iM_modul_activity=' . $act['iM_modul_activity'] . '&iM_activity=' . $act['iM_activity'] . '&' . $this->main_table_pk . '=' . $peka . '&raw_id='.$raw_id.'&iupb_id=' . $iupb_id . '&cKode=' . $rowData['cKode'] . '&company_id=' . $this->input->get('company_id') . '&group_id=' . $this->input->get('group_id') . '&modul_id=' . $this->input->get('modul_id') . ' \')"  id="button_approve_"' . $this->url . '"  class="ui-button-text icon-save" >Confirm Release</button>';
                        ...
                        case '2':
                            ...
                            case '3':
                                ...
                                if ($getLastStatusApprove) {
                                    // print_r($act);
                                    if($act['iSort'] == 3){
                                        $sButton .= /*$confirm*/ $release;
                                    } else {
                                        $sButton .= $confirm_release;
                                    }
    ...
    public function getLastFinish($idS)
    {

        $sql = 'select a.*, date(tMarkedAsFinished) as tMarkedAsFinished_short
                from hrd.ss_raw_problems a
                where a.id in (' . $idS . ')
                order by a.tMarkedAsFinished DESC limit 1 ';
        /*echo $sql;*/
        $dLast = $this->db->query($sql)->row_array();
        return $dLast;
    }
    

    public function getLastTags($repo, $d1, $d2)
    {

        //$repo = explode('/', $repo);
        $getEnd = end(explode('/', $repo));
        if ($getEnd == 'tags') {
            $tags = '';
        } else {
            //$tags = '/tags';
        ...
        $md2 = substr($d2, 0, 7);
        $date = "{" . $d2 . "}:{" . $d1 . "}";
        //$cmd = "svn log -v  --username machine --password irobot  -r ".$date." $repo | grep -B 2 'A\ \/tags'|head -1";
        $cmd = "svn log -v  --username machine --password irobot  -r " . $date . " $repo$tags|grep -E '" . $md1 . "|" . $md2 . "'|sed -n '/\(^r\|^\ *A\)/p'|head -1";
        // echo $cmd;
        // echo '<br>';
        /*$cmd = "svn log -v  --username machine --password irobot  -r ".$date." $repo|grep -B 2 'A\ \/tags'|head -1 2>&1";*/
        exec($cmd, $output, $return_var);
        /* echo $cmd; */
        //echo "<br>";

        $oExplode = explode('|', $output[0]);
        $timeZ = $oExplode[2];

        $oExplode = explode('+', $timeZ);
        $timeDate = trim($oExplode[0]);
