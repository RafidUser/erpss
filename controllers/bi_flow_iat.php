...
class bi_flow_iat extends MX_Controller { 
    ...
    function index($action = '') {
        ...
        $grid->addList('del','upload_dok','vModul','mKeterangan','dSubmit','iValidate','iSubmit'); 
        $grid->setSortBy('id');
        $grid->setSortOrder('DESC');  

        // $grid->addFields('upload_dok','vModul','mKeterangan','parameter','dSubmit');
        $grid->addFields('upload_dok','vModul','mKeterangan','dSubmit', 'mFeedback', 'form_history');
        ...
        switch ($action) {
            ...
            case 'delete':
                $postData = $this->input->post();
                    ...
                    $this->db->where(array('id' => $get_id['id'], 'cKode_iat' => $kode_iat));
                    $this->db->update('hrd.biflow_iat_log', $up);
                    // echo print_r($kode_iat);exit;
    ...
    function searchBox_bi_flow_iat_vKodeRef($rowData, $id) {
    ...
    function listBox_bi_flow_iat_upload_dok($value, $pk, $name, $rowData) {
    ...
    function listBox_bi_flow_iat_validasi($value, $pk, $name, $rowData){
    ...
    function listBox_bi_flow_iat_del($value, $pk, $name, $rowData){
        // print_r($this->input->get());exit;
    ...
    function insertBox_bi_flow_iat_mKeterangan($field, $id) {
    ...
    function updateBox_bi_flow_iat_mKeterangan($field, $id, $value, $rowData) {
        $o = '<input type="hidden" name="cKode" value="'.$this->input->get('cKode').'">';
        $o .= '<input type="hidden" name="isdraft" id="isdraft">';
        $o .= '<div class="col-sm-6">';
    ...
    function insertBox_bi_flow_iat_upload_dok($field, $id)
    {   
    ...
    function updateBox_bi_flow_iat_upload_dok($field, $id, $value, $rowData)
    {   
        // get data file
        $sql = "SELECT *
                FROM hrd.biflow_iat_file a
    ...
    function insertBox_bi_flow_iat_parameter($field, $id) {
    ...
    function updateBox_bi_flow_iat_parameter($field, $id, $value, $rowData) {
        $data['table']      = "hrd.biflow_iat_param";
        $data['table_key']  = "cKode_iat";
        $data['id']         = $id;
        $data['field']      = $field;
    ...
    function insertBox_bi_flow_iat_dSubmit($field, $id) {
        $o = '<b>Auto after submit</b>';

        return $o;
    }
    
    function updateBox_bi_flow_iat_dSubmit($field, $id, $value, $rowData) {
        $o = '<b>Auto after submit</b>';
    ...
    function insertBox_bi_flow_iat_mFeedback($field, $id) {
    ...
    function updateBox_bi_flow_iat_mFeedback($field, $id, $value, $rowData) {
        // print_r($this->input->get());exit;
        $data = $this->input->get();
        $id = $data['id'];
    ...
    function insertBox_bi_flow_iat_form_history($field, $id)
    {
        $o = '<b> - </b>';
    ...
    function updateBox_bi_flow_iat_form_history($field, $id, $value, $rowData)
    {
        $data['id'] = $id;
        $data['field'] = $field;
    ...
    function before_update_processor($row, $postData) {
        ...
        if ($postData['isdraft'] == 'true') {
            ...
            foreach ($rows as $key => $row) {
                ...
                // $this->db->where(array('id' =>  $get_id['id'], 'cKode_iat' =>  $kode_iat, 'lDeleted' => 0/*, 'cKode_br_log' => */));
